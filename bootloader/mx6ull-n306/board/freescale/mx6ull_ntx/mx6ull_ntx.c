...
static iomux_v3_cfg_t const ntx_pads[] = {
	MX6_PAD_LCD_DATA22__GPIO3_IO27 | MUX_PAD_CTRL(NO_PAD_CTRL), /* FL_EN */
	MX6_PAD_LCD_DATA09__GPIO3_IO14 | MUX_PAD_CTRL(NO_PAD_CTRL), /* EP_VMAX_ON */
	MX6_PAD_CSI_DATA02__GPIO4_IO23 | MUX_PAD_CTRL(NO_PAD_CTRL), /* WIFI_3V3_ON */
	MX6_PAD_SNVS_TAMPER8__GPIO5_IO08 | MUX_PAD_CTRL(I2C_PAD_CTRL),
};
...
int board_init(void)
{
	...
	setup_gpmi_nand();
#endif
	imx_iomux_v3_setup_multiple_pads(ntx_pads, ARRAY_SIZE(ntx_pads));
...
int board_late_init(void)
{
...

	{
		...
		fuse_read(0, 3, &val);
		printf ("fuse bank 0 offset 3 is 0x%08X\n", val); 
		if (0x300000 != (val & 0x300000)) {
			printf ("Disable tamper 0-9, used as GPIO\n"); 
			val |= 0x300000;
			if(fuse_prog(0, 3, val))
