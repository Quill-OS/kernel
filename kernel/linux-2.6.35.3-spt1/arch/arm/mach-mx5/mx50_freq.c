...
// Customized for Samsung LPDDR1 (K4X2G323PC) and Hynix LPDDR1 (H5MS2G22AFR)
...
int update_ddr_freq(int ddr_rate)
{
	...
	if (ddr_rate == LP_APM_CLK) {
		if (mx50_ddr_type == MX50_LPDDR2) {
			iram_ddr_settings[0][0] = ARRAY_SIZE(lpddr2_24);
			...
			iram_ddr_settings[0][0] = ARRAY_SIZE(mddr_24);
			for (i = 0; i < iram_ddr_settings[0][0]; i++) {
				iram_ddr_settings[i + 1][0]
								= mddr_24[i][0];
				iram_ddr_settings[i + 1][1]
