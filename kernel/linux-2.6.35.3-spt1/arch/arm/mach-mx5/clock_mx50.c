...
({ \
	long elapse = 0; \
	long timeout_usec = (timeout)  / 1000 ; \
	int result = 1; \
	while (!(exp)) { \
		if (elapse > timeout_usec) { \
			result = 0; \
			break; \
		} \
		elapse++; \
...
static int apll_enable(struct clk *clk)
{
	...
	if (!WAIT(__raw_readl(apll_base + MXC_ANADIG_PLLCTRL)
		  & MXC_ANADIG_APLL_LOCK, SPIN_DELAY /* 80000 */)) {
		printk("Error!!!!! Can't lock 480MHz PLL.\n");
		printk("%s: MXC_ANADIG_MISC=%08X\n", __func__, __raw_readl(apll_base + MXC_ANADIG_MISC));
		printk("%s: MXC_ANADIG_PLLCTRL=%08X\n", __func__, __raw_readl(apll_base + MXC_ANADIG_PLLCTRL));
...
#if 1 /* E_BOOK *//* for Serial Switcher 2011/1/18 */
	_REGISTER_CLOCK("mxcintswuart.0", NULL, uart1_clk[0]),
	_REGISTER_CLOCK("mxcintswuart.1", NULL, uart2_clk[0]),
#endif
...
int __init mx50_clocks_init(unsigned long ckil, unsigned long osc, unsigned long ckih1)
{
	...
	clk_set_parent(&ssi_ext1_clk, &ssi_lp_apm_clk);
