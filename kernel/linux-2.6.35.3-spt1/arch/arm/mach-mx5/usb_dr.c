...
static bool enable_otg_vbus_wake_up_flag = false;
static bool current_state_of_wake_up_enable = false;
static void usbotg_clock_gate(bool on);

void enable_otg_vbus_wake_up(bool enable)
{
	if(enable == current_state_of_wake_up_enable){
		return;
	}
	
	usbotg_clock_gate(1); //ON
	
	if(enable && enable_otg_vbus_wake_up_flag){
		current_state_of_wake_up_enable = true;
		USBCTRL |= UCTRL_OWIE;
		USB_PHY_CTR_FUNC |= USB_UTMI_PHYCTRL_CONF2;
		USBCTRL_HOST2 |= UCTRL_H2OVBWK_EN;
	}else if( !enable ){
		current_state_of_wake_up_enable = false;
		USBCTRL &= ~UCTRL_OWIE;
		USB_PHY_CTR_FUNC &= ~USB_UTMI_PHYCTRL_CONF2;
		USBCTRL_HOST2 &= ~UCTRL_H2OVBWK_EN;
	}
}
EXPORT_SYMBOL(enable_otg_vbus_wake_up);
...
static void _device_wakeup_enable(struct fsl_usb2_platform_data *pdata, bool enable)
{
	...
	if(enable){
		enable_otg_vbus_wake_up_flag = true;
		return;
	}else{	
		enable_otg_vbus_wake_up_flag = false;
		current_state_of_wake_up_enable = false;
	}
