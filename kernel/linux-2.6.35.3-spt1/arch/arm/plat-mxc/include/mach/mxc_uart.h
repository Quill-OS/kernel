...
/*-----------------------------------------------
  Serial Switcher
  ----------------------------------------------*/
#define MXC_SSW_MODE_INPUT	0
#define MXC_SSW_MODE_SERIAL 	1

#define MXC_SSW_MAXDEV	2

#define MXC_SSW_RECVBUFSIZE 1024
/*
 */
struct mxc_ssw_recept {
  void (*connect)(void *);
  void (*recv)(void *,unsigned char);
  void (*suspend)(void *);
  void (*resume)(void *);
  void (*disconnect)(void *);
  char recvbuf[MXC_SSW_RECVBUFSIZE];
  int recvbuf_index;
};
...
struct mxc_ssw_link {
  struct mxc_ssw_recept *inputdev;
  void *inputarg;
  struct mxc_ssw_port *serialdev;
  void *serialarg;
  int flag;
  int ssw_mode;
  int serial_portno;
...
void mxc_ssw_set_receive_buffer(int port,char *buff,int size);
...
int mxc_ssw_get_rts_edge(void);
void mxc_ssw_clear_rts_edge(void);
void mxc_ssw_port_close(int );
void mxc_ssw_port_open(int );
void mxc_ssw_enable_irq_wakeup(int);
void mxc_ssw_disable_irq_wakeup(int);
#endif
