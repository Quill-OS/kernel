...
#if 1 /* E_BOOK *//* fix for no blink in shutdown inpgress 2011/08/05 */
#define RESET_TIMEOUT 1000  /* usec */
#endif
...
int pwm_config(struct pwm_device *pwm, int duty_ns, int period_ns)
{
	...
	if (!(cpu_is_mx1() || cpu_is_mx21())) {
		...
		if ( duty_ns > 0 ) {
		  int timeout=0;
		  writel(0x08,pwm->mmio_base + MX3_PWMCR);
		  while((readl(pwm->mmio_base + MX3_PWMCR)&0x8)){
		    if ( timeout > RESET_TIMEOUT) {
		      printk("PWM: reset timeout occured \n");
		      break;
		    }
		    timeout++;
		  }
