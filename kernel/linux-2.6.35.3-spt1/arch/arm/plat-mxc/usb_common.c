...
int fsl_usb_host_init(struct platform_device *pdev)
{
	...
	if (cpu_is_mx50())
	{
		u32 phy_ctrl_0 = USBH1_PHY_CTRL0;
		phy_ctrl_0 &= ~(1<<11); /* Release PHY reset */
		phy_ctrl_0 |= (1<<12); /* Release low_power mode (start calibration) */
		USBH1_PHY_CTRL0 = phy_ctrl_0;
		pr_debug("Host PHY calibration\n");
	}
...
void fsl_usb_host_uninit(struct fsl_usb2_platform_data *pdata)
{
	...
	{
		u32 phy_ctrl_0 = USBH1_PHY_CTRL0;
		phy_ctrl_0 &= ~(1<<12);     /* Force low_power mode */
		phy_ctrl_0 |= (1<<11);      /* assert PHY reset */
		USBH1_PHY_CTRL0 = phy_ctrl_0;
		pr_debug("Host PHY enter low power & reset\n");
	}
