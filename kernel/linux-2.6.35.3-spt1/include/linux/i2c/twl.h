...
#define TWL6030_SIMCTRL			0xEC
#define VUSIM_AUTO_OFF			(0x1 << 3)
#define STS_SIM				0x1
...
#define TWL6030_POWER_SOURCE_NONE		0
#define TWL6030_POWER_SOURCE_USB_PC		1
#define TWL6030_POWER_SOURCE_USB_CHARGER	2
...
struct twl4030_bci_platform_data {
	...
	/* Voltage Averaging */
	unsigned int initial_v_no_samples;
	unsigned int initial_fuelgauge_mode;
	
	/* Battery Capacity Callbacks */
	unsigned int (*vprime_callback)(int voltage_uV, int current_uA);
	unsigned int (*capacity_callback)(int voltage_uV);

	/* Voltage Channel to sample for VBAT */
	int vbat_channel;
	
	/* Disable the Average current calcs */
	int disable_current_avg;
	
	/* Disable the charging of backup batt */
	int disable_backup_charge;

	/* Number if CC readings before recal */
	int fuelgauge_recal;
...
struct twl4030_usb_data {
	...
	int		(*phy_set_clock)(struct device *dev, int on);
		...
		/* Called when a USB connection is detected should be initialised
         * when the default behaviour of enabling LDOUSB is not wanted
         */
	int		(*board_control_phy_power)(struct device *dev, int on);

	/* Called when the state of VBUS changes */
	int		(*board_control_vbus_power)(struct device *dev, int on);
};

struct twl4030_vbus_data {
	/* Called when the state of VBUS changes */
	int		(*board_control_vbus_power)(struct device *dev, int on);
};
