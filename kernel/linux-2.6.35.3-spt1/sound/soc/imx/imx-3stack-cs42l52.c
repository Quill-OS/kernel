/*
 * imx-3stack-cs42l52.c  --  i.MX 3Stack Driver for Freescale CS42L52 Codec
 *
...
static unsigned int cs42l52_rates[] = {
	0,
	8000,
	11025,
	12000,
				      ...
				      struct snd_pcm_hw_params *params)
{
	...
	snd_soc_dai_set_sysclk(codec_dai, CS42L52_SYSCLK, priv->sysclk, 0);

#if CS42L52_SSI_MASTER
...
/*
 * imx_3stack CS42L52 audio DAI opserations.
 */
...
static void imx_3stack_init_dam(int ssi_port, int dai_port)
{
	...
	/* CS42L52 uses SSI1 or SSI2 via AUDMUX port dai_port for audio */
...
static const struct snd_soc_dapm_route audio_map[] = {
	...
	/* Mic Jack --> MICx (with automatic bias) */
	{"MICA", NULL, "Mic Jack"},
	{"MICB", NULL, "Mic Jack"},

	/* Line In Jack --> INPUT1x */
	{"INPUT1A", NULL, "Line In Jack"},
	{"INPUT1B", NULL, "Line In Jack"},

	/* HPx --> Headphone Jack */
	{"Headphone Jack", NULL, "HPA"},
	{"Headphone Jack", NULL, "HPB"},
...
static int cs42l52_jack_func;
static int cs42l52_spk_func;
static int cs42l52_line_in_func;
...
static ssize_t show_headphone(struct device_driver *dev, char *buf)
{
	...
	if (hp_status == 0)
		snprintf(buf, PAGE_SIZE, "speaker\n");
	else
		snprintf(buf, PAGE_SIZE, "headphone\n");
...
static int cs42l52_get_jack(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = cs42l52_jack_func;
	return 0;
}

static int cs42l52_set_jack(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if (cs42l52_jack_func == ucontrol->value.enumerated.item[0])
		return 0;

	cs42l52_jack_func = ucontrol->value.enumerated.item[0];
	if (cs42l52_jack_func)
...
static int cs42l52_get_spk(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = cs42l52_spk_func;
	return 0;
}

static int cs42l52_set_spk(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if (cs42l52_spk_func == ucontrol->value.enumerated.item[0])
		return 0;

	cs42l52_spk_func = ucontrol->value.enumerated.item[0];
	if (cs42l52_spk_func)
...
static int cs42l52_get_line_in(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = cs42l52_line_in_func;
	return 0;
}

static int cs42l52_set_line_in(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if (cs42l52_line_in_func == ucontrol->value.enumerated.item[0])
		return 0;

	cs42l52_line_in_func = ucontrol->value.enumerated.item[0];
	if (cs42l52_line_in_func)
...
static const struct snd_kcontrol_new cs42l52_machine_controls[] = {
	SOC_ENUM_EXT("Jack Function", cs42l52_enum[0], cs42l52_get_jack,
		     cs42l52_set_jack),
	SOC_ENUM_EXT("Speaker Function", cs42l52_enum[1], cs42l52_get_spk,
		     cs42l52_set_spk),
	SOC_ENUM_EXT("Line In Function", cs42l52_enum[1], cs42l52_get_line_in,
		     cs42l52_set_line_in),
};
...
static const struct soc_enum asrc_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(asrc_function), asrc_function),
};
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	asrc_ssi_data.output_sample_rate = cs42l52_rates[asrc_func];
...
static int imx_3stack_cs42l52_init(struct snd_soc_codec *codec)
{
	int i, ret;
	...
	for (i = 0; i < ARRAY_SIZE(cs42l52_machine_controls); i++) {
		ret = snd_ctl_add(codec->card,
				  snd_soc_cnew(&cs42l52_machine_controls[i],
					       codec, NULL));
		if (ret < 0)
			return ret;
	}
...
static struct snd_soc_dai_link imx_3stack_dai = {
	.name			= SOC_CS42L52_NAME " dai-link",
	.stream_name	= SOC_CS42L52_NAME " stream",
	.codec_dai		= &soc_cs42l52_dai,
	.init			= imx_3stack_cs42l52_init,
	.ops			= &imx_3stack_ops,
};
...
static struct snd_soc_device imx_3stack_snd_devdata = {
	...
	.codec_dev	= &soc_codec_cs42l52_device,
};

static int __devinit imx_3stack_cs42l52_probe(struct platform_device *pdev)
{
	struct mxc_audio_platform_data *plat = pdev->dev.platform_data;
	struct imx_3stack_priv *priv = &card_priv;
	struct snd_soc_dai *cs42l52_cpu_dai;
	int ret = 0;
	...
	if (plat->src_port == 2)
		cs42l52_cpu_dai = imx_ssi_dai[2];
	else
		cs42l52_cpu_dai = imx_ssi_dai[0];

	imx_3stack_dai.cpu_dai = cs42l52_cpu_dai;
	...
	/* The CS42L52 has an internal reset that is deasserted 8 SYS_MCLK
...
static int imx_3stack_cs42l52_remove(struct platform_device *pdev)
{
	struct mxc_audio_platform_data *plat = pdev->dev.platform_data;
	struct imx_3stack_priv *priv = &card_priv;
...
static struct platform_driver imx_3stack_cs42l52_audio_driver = {
	.probe = imx_3stack_cs42l52_probe,
	.remove = imx_3stack_cs42l52_remove,
	.driver = {
		   .name = "imx-3stack-cs42l52",	/* platform_device key-name */
		   },
};
...
static int __init imx_3stack_init(void)
{
	...
	ret = platform_driver_register(&imx_3stack_cs42l52_audio_driver);
	if (ret)
		return -ENOMEM;
...
{
	platform_driver_unregister(&imx_3stack_cs42l52_audio_driver);
...
MODULE_DESCRIPTION("CS42L52 Driver for i.MX 3STACK");
MODULE_LICENSE("GPL");
