...
static	int
dma_new_period(struct snd_pcm_substream* substream)
{
	struct	snd_pcm_runtime*	runtime = substream->runtime;
	struct	mxc_runtime_data*	prtd = runtime->private_data;
	...
	for (index = 0; index < runtime->periods; index++) {
		mxc_dma_requestbuf_t*	request = prtd->dma_alloc + index;
		dma_addr_t				dma_addr = runtime->dma_addr + dma_size * (index % runtime->periods);

		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			request->src_addr = dma_addr;
		} else {
			request->dst_addr = dma_addr;
		}
		request->num_of_bytes = dma_size;
	}

	result = mxc_dma_config(
					prtd->dma_wchannel,
					prtd->dma_alloc,
					runtime->periods,
					(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) ? MXC_DMA_MODE_WRITE: MXC_DMA_MODE_READ
				);
	switch (result) {
	case -EBUSY:
		printk(KERN_INFO "%s: already DMA configured: %d\n", __FILE__, prtd->dma_wchannel);
		/* fall through */
	case 0:
		break;

	default:
		printk(KERN_ERR "%s: mxc_dma_config(%d): %d\n", __FILE__, prtd->dma_wchannel, result);
		return result;
	}

	result = mxc_dma_enable(prtd->dma_wchannel);
	if (result < 0) {
		printk(KERN_ERR "%s: mxc_dma_enable(%d): %d\n", __FILE__, prtd->dma_wchannel, result);
		return result;
	}
...
static	 void
audio_dma_irq(struct snd_pcm_substream* substream)
{
	struct snd_pcm_runtime*		runtime = substream->runtime;
	struct mxc_runtime_data*	prtd = runtime->private_data;
	int							result;
	...
	result = mxc_dma_config(
					prtd->dma_wchannel,
					prtd->dma_alloc + prtd->periods,
					1,
					(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) ? MXC_DMA_MODE_WRITE: MXC_DMA_MODE_READ
				);
	if (result < 0) {
		printk(KERN_ERR "%s: %s: mxc_dma_config: %d\n", __FILE__, __func__, result);
...
static	int
imx_pcm_prepare(struct snd_pcm_substream *substream)
{
	struct	snd_pcm_runtime*	runtime = substream->runtime;
	struct	mxc_runtime_data*	prtd = runtime->private_data;
	mxc_sdma_channel_params_t*	dma_params = mxc_sdma_get_channel_params(prtd->dma_ch);
	int							channel = 0;

	if (prtd->dma_alloc != NULL) {
		mxc_dma_free(prtd->dma_wchannel);
		kfree(prtd->dma_alloc);
		prtd->dma_alloc = NULL;
	}
	...
	prtd->dma_alloc = kzalloc(sizeof(mxc_dma_requestbuf_t) * dma_params->chnl_params.bd_number, GFP_KERNEL);
	if (prtd->dma_alloc == NULL) {
		return -ENOMEM;
	}
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (prtd->asrc_enable == 1) {
			...
			prtd->dma_alloc->num_of_bytes = 0x40;
			channel =
			    mxc_dma_request_ext(prtd->dma_ch,
						"ALSA TX SDMA", &info);

			mxc_dma_config(channel, prtd->dma_alloc, 1, MXC_DMA_MODE_WRITE);
	...
	return mxc_dma_callback_set(channel, (mxc_dma_callback_t)audio_dma_irq, substream);
}



static	int
imx_pcm_hw_params(struct snd_pcm_substream* substream, struct snd_pcm_hw_params* params)
{
	struct	snd_pcm_runtime*		runtime = substream->runtime;
	struct	mxc_runtime_data*		prtd = runtime->private_data;
	struct	snd_soc_pcm_runtime*	rtd = substream->private_data;

	prtd->dma_ch = imx_get_sdma_transfer(params_format(params), rtd->dai->cpu_dai->id, substream);
	if (prtd->dma_ch < 0) {
...
static	int
imx_pcm_hw_free(struct snd_pcm_substream* substream)
{
	struct	snd_pcm_runtime*	runtime = substream->runtime;
	struct	mxc_runtime_data*	prtd = runtime->private_data;
...
static	int
imx_pcm_trigger(struct snd_pcm_substream* substream, int cmd)
{
