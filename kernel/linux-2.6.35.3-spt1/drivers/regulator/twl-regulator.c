...
#define PREQ1_RES_ASS_A_SMPS3	BIT(3)
...
static void twl_regulator_early_suspend(struct early_suspend *h)
{
	if ( !gpio_get_value(BOARD_ID) )  // BB
	{
		return;
	} 
	...
	if (h->pm_mode == EARLY_SUSPEND_MODE_NORMAL) {
		// When i.MX508 enters the standby state, V2.9 is turned off.
		
		u8 value;
		
		//Set PREQ1_RES_ASS_A
		twl_i2c_read_u8(TWL_MODULE_RTC, &value, TWL6025_PREQ1_RES_ASS_A);
		value |= PREQ1_RES_ASS_A_SMPS3;
		twl_i2c_write_u8(TWL_MODULE_RTC, value, TWL6025_PREQ1_RES_ASS_A);

		//Set SMPS4_CFG_TRANS		
		twl_i2c_write_u8(TWL_MODULE_RTC, 0x01, TWL6025_SMPS4_CFG_TRANS);
	}
}

static void twl_regulator_late_resume(struct early_suspend *h)
{
	// When i.MX508 enters the standby state, V2.9 isn't turned off.
	if ( !gpio_get_value(BOARD_ID) )  // BB
	{
		return;
	} 
	...
	if (h->pm_mode == EARLY_SUSPEND_MODE_NORMAL) {
		...
		//Set SMPS4_CFG_TRANS		
		twl_i2c_write_u8(TWL_MODULE_RTC, 0x05, TWL6025_SMPS4_CFG_TRANS);
	}

}

static struct early_suspend twl_regulator_earlysuspend = {
	.level = EARLY_SUSPEND_LEVEL_DISABLE_FB,
	.suspend = twl_regulator_early_suspend,
	.resume = twl_regulator_late_resume,
};
#endif
...
{
#ifdef CONFIG_EARLYSUSPEND
	register_early_suspend(&twl_regulator_earlysuspend);
#endif
...
{
#ifdef CONFIG_EARLYSUSPEND
	unregister_early_suspend(&twl_regulator_earlysuspend);
#endif
