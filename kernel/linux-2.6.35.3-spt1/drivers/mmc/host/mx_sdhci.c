...
#if 1 /* E_BOOK *//* for power saving 2011/04/05 */
...
#if 1 /* E_BOOK *//* Slot Power control 2010/12/20 */
#include <mach/gpio.h>
#endif
...
#ifdef DELAYED_CD  /* E_BOOK *//* for power saving 2011/04/04 */
static int delay_cd_flag=0;
//static int resume_cd_flag=0;
#endif
#if 1 /* E_BOOK *//* for suspend/resume 2011/06/13 */
extern int dont_suspend;
...
#if 1 /* E_BOOK *//* Slot Power control 2010/12/20 */
#define SD1_CDPWR (4*32+12) /* GPIO_5_12 */
#endif
#if 1 /* E_BOOK *//* SD Access LED 2011/04/07 */
#define SD1_LED	(1*32+16) /* GPIO_2_16 */
#endif
...
#if 1 /* E_BOOK *//* fix for Electrostatic error 2011/08/06 */
static int sd_retry_cnt=0;
#define SD_RETRY_MAX (10)
#endif

#if 1 /* E_BOOK *//* for TEST 2011/05/11 */
int mxc_mmc_busycheck(struct mmc_host *mmc)
{
	struct sdhci_host *host;
	unsigned int ret;

	host = mmc_priv(mmc);
	ret = readl(host->ioaddr+SDHCI_PRESENT_STATE);
	//printk("mxc_mmc_busycheck: PRESET_STATE=0x%x\n",ret);
	return (ret & SDHCI_DATA_ACTIVE)? 1: 0;
}
void mxc_mmc_clk_always_on(struct mmc_host *mmc)
{
  struct sdhci_host *host;
  unsigned int ret;

  host = mmc_priv(mmc);
  host->plat_data->clk_always_on = 1;
}
void mxc_mmc_clk_always_off(struct mmc_host *mmc)
{
  struct sdhci_host *host;
  unsigned int ret;

  host = mmc_priv(mmc);
  host->plat_data->clk_always_on = 0;
...
#endif
#if 1 /* E_BOOK *//* for TEST 2011/07/07 */
int mxc_get_cd_status(struct mmc_host *mmc)
{
  struct sdhci_host *host;
  unsigned int ret;
...
EXPORT_SYMBOL(mxc_get_cd_status);
#endif
#if 1 /* E_BOOK *//* bugfix for FAT deadlock 2011/07/20 */
int mxc_get_cd_status_real(struct mmc_host *mmc)
{
  struct sdhci_host *host;
  unsigned int ret;
...
EXPORT_SYMBOL(mxc_get_cd_status_real);
#endif
...
#ifdef DELAYED_CD /* E_BOOK *//* for power saving 2011/04/04 */
	schedule_delayed_work(&host->cd_wq,0);
#else
...
#if 1 /* E_BOOK *//* for TEST 2011/06/13 */
	if ( host->id != 3 ) {
	  if ( (host->clock == 0) || (host->plat_data->status(host->mmc->parent) != 0) 
	       || ((host->flags & SDHCI_CD_PRESENT)==0)) {
	    cmd->error = -ENOMEDIUM;
	    queue_work(host->workqueue, &host->finish_wq);
	    return;
	  }
...
#if 0 /* E_BOOK *//* for TEST 2011/05/10 */
	/*
...
#if 0 /* E_BOOK *//* delete for eMMC 2011/03/28 */
...
#if 1 /* E_BOOK *//* for power saving 2011/04/04 */
static unsigned long long sdhci_poweroff_time=0;
static int first=1;
#endif
...
#if 1 /* E_BOOK *//* for power saving, delayed power on  2011/04/04 */
	if ( host->id == 0 ) {
	  //printk("SDHC:sdhci_set_power = %d\n",power);
	  if ((power == 65535) || ((host->flags & SDHCI_CD_PRESENT)==0)) {
	    printk(KERN_DEBUG "SDHC:sdhci_set_power:slot pwr off\n");
	    gpio_set_value(SD1_CDPWR,1);
	    sdhci_poweroff_time=get_jiffies_64();
	  } else {
	    unsigned long long delta;
	    if ( first == 0 ) {
	      delta = get_jiffies_64() - sdhci_poweroff_time;
	      if ( delta < 100 ) {
		//printk("SDHCI:sdhci_set_power: sleep %d msec\n",(int)(100-delta));
		msleep(100 - delta);
	      }
	    }
	    first=0;
	    printk(KERN_DEBUG "SDHC:sdhci_set_power:slot pwr on\n");
	    gpio_set_value(SD1_CDPWR,0);
	  }
	}
#endif	  
	host->power = power;
...
static void sdhci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	/* Enable the clock */
	if (!host->plat_data->clk_flg) {
#if 0 /* E_BOOK *//* for TEST 2011/1/20 */
	  {
	    unsigned int clk;
 ...
 *                                                                           *
\*****************************************************************************/
#if 0 /* E_BOOK *//* TEST for Electrostatic 2011/08/06 */
static elecpalse=0;
static ssize_t sdhci_elecpalse_set(struct device *dev, struct device_attribute *attr, char *buf)
{
  int flag;
  flag = simple_strtol(buf,NULL,0);
  if ( flag ) {
    printk("SDHCI:Electric palse on\n");
    elecpalse=1;
  } else {
    printk("SDHCI:Electric palse off\n");
    elecpalse=0;
  }
  return strlen(buf);
}
static DEVICE_ATTR(elec, 0644, NULL, sdhci_elecpalse_set);
#endif
...
static void sdhci_cmd_irq(struct sdhci_host *host, u32 intmask)
{
...
#if 0 /* E_BOOK *//* TEST for Electrostatic 2011/08/12 */
	if ( (host->id == 0) &&  (elecpalse!=0)) {
	  printk("SDHC:elec pulse\n");
	  intmask |= 0xe0000;
	  elecpalse=0;
	}
#endif
	...
	if (host->cmd->error) {
	  if ( (host->id == 0) 
	       && ((host->cmd->error == -EILSEQ) /* || (host->cmd->error == -ETIMEDOUT)*/)
		&& (host->flags & SDHCI_CD_PRESENT)) {
	    if (sd_retry_cnt < SD_RETRY_MAX) {
	      sd_retry_cnt++;
	      printk(KERN_INFO "SDHC:sdhci_cmd_irq:reset & retry (intmask=%x)\n",intmask);
	      //sdhci_reset(host, SDHCI_RESET_CMD);
	      //sdhci_reset(host, SDHCI_RESET_DATA);
	      sdhci_reset(host, SDHCI_RESET_ALL);
	      host->cmd->error = -EAGAIN;
	    } else {
	      printk(KERN_INFO "SDHC:sdhci_cmd_irq:retry over (intmask=%x)\n",intmask);
	    }
	  }
	  queue_work(host->workqueue, &host->finish_wq);
	}
...
static void sdhci_data_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (!host->data) {
		...
		if (intmask & SDHCI_INT_DATA_END)
			return;
#if 1 /* E_BOOK *//* for bugfix 2011/08/01 */
		if ((intmask & SDHCI_INT_DMA_END) &&
		    (!(intmask & SDHCI_INT_DATA_END))) {
			writel(readl(host->ioaddr + SDHCI_DMA_ADDRESS),
			       host->ioaddr + SDHCI_DMA_ADDRESS);
			printk(KERN_INFO "Occured DMA intr after DATA END intr\n");
			return;
		}
#endif
...
#if 0 /* E_BOOK *//* TEST for Electrostatic 2011/08/14 */
	if ( (host->id == 0) &&  (elecpalse!=0)) {
	  printk("SDHC:data:elec pulse\n");
	  intmask |= 0x600000;
	  elecpalse=0;
	}
#endif
	...
	if (host->data->error) {
	  if ( (host->id == 0) 
	       && ((host->data->error == -EILSEQ) || (host->data->error == -ETIMEDOUT))
		&& (host->flags & SDHCI_CD_PRESENT)) {
	    if (sd_retry_cnt < SD_RETRY_MAX) {
	      sd_retry_cnt++;
	      printk(KERN_INFO "SDHC:sdhci_data_irq:reset & retry (intmask=%x)\n",intmask);
	      //sdhci_reset(host, SDHCI_RESET_CMD);
	      //sdhci_reset(host, SDHCI_RESET_DATA);
	      sdhci_reset(host, SDHCI_RESET_ALL);
	      host->data->error = -EAGAIN;
	    } else {
	      printk(KERN_INFO "SDHC:sdhci_data_irq:retry over (intmask=%x)\n",intmask);
	    }
	  }
	  sdhci_finish_data(host);
	}
...
static void esdhc_cd_callback(struct work_struct *work)
{
	...
	unsigned int cd_status = 0;
#ifdef DELAYED_CD /* E_BOOK *//* for power saving 2011/04/04 */
	struct sdhci_host *host = container_of(work, struct sdhci_host, cd_wq.work);
#else
...
#if 0 /* E_BOOK *//* delete for TEST 2011/06/09 */
	...
	cd_status = host->plat_data->status(host->mmc->parent);
#if 0 /* E_BOOK *//* Slot Power control 2010/12/20 */
	/* delete for Delayed power on . 2011/04/05 */
        if (cd_status) {
	  host->flags &= ~SDHCI_CD_PRESENT;
	  if ( host->id == 0 ) {
	    //printk("SDHC:slot pwr off\n");
	    gpio_set_value(SD1_CDPWR,1);
	  }
        } else {
	  host->flags |= SDHCI_CD_PRESENT;
	  if ( host->id == 0 ) {
	    //printk("SDHC:slot pwr on\n");
	    gpio_set_value(SD1_CDPWR,0);
	  }
        }
#else
	...
	if (!(host->flags & SDHCI_CD_PRESENT)) {
		printk(KERN_INFO
		       ...
		       mmc_hostname(host->mmc));
#if 1 /* E_BOOK *//* power off slot immediatry 2011/07/15 */
		if ( host->id == 0) {
		  printk(KERN_DEBUG "SDHC:cd_callback:slot pwr off\n");
		  gpio_set_value(SD1_CDPWR,1);
		  sdhci_poweroff_time=get_jiffies_64();
		}
#endif
	...
	if ( host->id == 0 ) {
	  //wake_unlock(&host->cd_work_wake_lock);
	}
#endif
}
...
#ifdef DELAYED_CD /* E_BOOK *//* for power saving 2011/04/04 */
	if ( host->id == 0 ) {
	  //wake_lock_timeout(&host->cd_work_wake_lock,HZ/2);
	  //wake_lock(&host->cd_work_wake_lock);
	  if ( delay_cd_flag ) {
	    //printk("sdhci_cd_irq: delayed schedule =%d\n",HZ/20);
	    //schedule_delayed_work(&host->cd_wq,HZ/20);
	    printk(KERN_DEBUG "sdhci_cd_irq: request resume detect\n");
	    host->mmc->resume_cd_flag=1;
	  } else {
	    dont_suspend=1;
	    printk(KERN_DEBUG "sdhci_cd_irq: non delayed schedule(%d)\n",dont_suspend);
	    schedule_delayed_work(&host->cd_wq,0);
	  }
	} else {
	  schedule_delayed_work(&host->cd_wq,0);
...
static int sdhci_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	DBG("Suspending...\n");
#if 1 /* E_BOOK *//* for DEBUG 2011/06/13 */
	printk(KERN_DEBUG "sdhci_suspend(id=%d,dont suspend=%d)\n",chip->hosts[0]->id,dont_suspend);
#endif

#if 1 /* E_BOOK *//* for detect check 2011/06/13 */
	if ( dont_suspend) {
	  printk(KERN_DEBUG "sdhci_suspend:do not suspend\n");
	  return -EBUSY;
	}
	if ( dont_suspend_by_IO ) {
	  printk(KERN_DEBUG "sdhci_suspend:do not suspend by IO\n");
	  return -EBUSY;
	}
#endif
#ifdef DELAYED_CD /* E_BOOK *//* for power saving 2011/04/04 */
	...
	for (i = 0; i < chip->num_slots; i++) {
		if (!chip->hosts[i])
			continue;
#if 1	/*E_BOOK*/
		if( pdev->id == 3 ) {		//No.3 is wifi
			//clear all interrupt_enable registers
			u32 ier;
			ier = readl(chip->hosts[i]->ioaddr + SDHCI_INT_ENABLE);
			ier &= ~SDHCI_INT_ALL_MASK;
			writel(ier,chip->hosts[i]->ioaddr + SDHCI_INT_ENABLE);
			writel(ier,chip->hosts[i]->ioaddr + SDHCI_SIGNAL_ENABLE);
		}
#endif
		free_irq(chip->hosts[i]->irq, chip->hosts[i]);
#if 1 /* E_BOOK *//* for wakeup on sleep and standby 2011/04/02 */
		if ( chip->hosts[i]->detect_irq ) {
		  enable_irq_wake(chip->hosts[i]->detect_irq);
		}
#endif
#if 1 /* E_BOOK *//* power off slot 2011/04/02 */
		if (chip->hosts[i]->mmc->index == 0 ) {
		  printk(KERN_DEBUG "SDHC: slot power off\n");
		  gpio_set_value(SD1_CDPWR,1);
		}
#endif
	}
...
static int sdhci_resume(struct platform_device *pdev)
{
	...
	DBG("Resuming...\n");
#if 1 /* E_BOOK *//* for DEBUG 2011/06/13 */
	printk(KERN_DEBUG "sdhci_resume\n");
#endif
...
#ifdef DELAYED_CD /* E_BOOK *//* for power saving 2011/04/04 */
	if ( chip->hosts[0]->id == 0) {
	  delay_cd_flag=0;
	  if ( chip->hosts[0]->mmc->resume_cd_flag ) {
	    chip->hosts[0]->mmc->resume_cd_flag=0;
	    dont_suspend=1;
	    printk(KERN_DEBUG "sdhci_resume: detect schedule\n");
	    schedule_delayed_work(&chip->hosts[0]->cd_wq,5);
	  }
	}
#endif
	return 0;
...
#ifdef DELAYED_CD /* for power saving . 2011/04/05 */
	INIT_DELAYED_WORK(&host->cd_wq, esdhc_cd_callback);
#else
...
#if 1 /* E_BOOK *//* for power saving 2011/04/05 */
	if ( host->id == 0 ) {
	  //wake_lock_init(&host->cd_work_wake_lock, WAKE_LOCK_SUSPEND, "sdhci_cd_work");
	}
#endif
...
#if 0 /* E_BOOK *//* TEST for Electroical ... 2011/08/12 */
	if( sysfs_create_file(&pdev->dev.kobj, &dev_attr_elec.attr) < 0 ) {
	  printk("SDHCI:sysfs create error\n");
	}
#endif
	return 0;
...
{
#if 1 /* E_BOOK *//* 2011/08/09 */
	printk(KERN_INFO DRIVER_NAME
	       ": MXC Secure Digital Host Controller Interface driver(all reset on error version)\n");
#else
	printk(KERN_INFO DRIVER_NAME
...
/* 2011/2/5 FY11 : Modified to call initialize func more earlier. */
