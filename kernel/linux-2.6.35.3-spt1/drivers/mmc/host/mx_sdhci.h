...
#define SDHCI_CD_TIMEOUT 	(1<<10)	/* cd timer is expired */
...
#ifdef DELAYED_CD
	struct delayed_work cd_wq;	/* card detection work queue */
#else
	...
	struct wake_lock cd_work_wake_lock; /* wake lock */
	//int delay_cd_flag; 	/* delayed cd flag */
	//int resume_cd_flag;	/* do delect after resume */
#endif
};
