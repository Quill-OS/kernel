...
static const u32 scalar_bit_map = 0x4785;
...
static const struct twl6025_scalar_channel twl6025_trim[17] = {
	{ TWL6025_GPADC_TRIM1, 0x7, TWL6025_GPADC_TRIM2, 0x07},
	{ 0x00, },
	{ TWL6025_GPADC_TRIM3, 0x1F, TWL6025_GPADC_TRIM4, 0x3F},
	{ 0x00, },
	{ 0x00, },
	{ 0x00, },
	{ 0x00, },
	{ TWL6025_GPADC_TRIM7, 0x1F, TWL6025_GPADC_TRIM8, 0x1F },
	{ TWL6025_GPADC_TRIM9, 0x0F, TWL6025_GPADC_TRIM10, 0x1F },
	{ TWL6025_GPADC_TRIM11, 0x0F, TWL6025_GPADC_TRIM12, 0x1F },
	{ TWL6025_GPADC_TRIM13, 0x0F, TWL6025_GPADC_TRIM14, 0x1F },
	{ 0x00, },
	{ 0x00, },
	{ 0x00, },
	{ TWL6025_GPADC_TRIM15, 0x0f, TWL6025_GPADC_TRIM16, 0x1F },
	{ 0x00, },
	{ 0x00 ,},
};
...
static const struct twl6030_ideal_code twl6025_ideal[17] = {
	{463,	2982},	/* CHANNEL 0 */
	{328,	3604},	/* CHANNEL 1 */
	{221,	3274},	/* CHANNEL 2 */
	{328,	3604},	/* CHANNEL 3 */
					  ...
					  u8 reg, int channel)
{
	...
	if (twl_features() & TWL6025_SUBCLASS) {
		...
		msb = twl6030_gpadc_read(gpadc, TWL6025_GPCH0_MSB);
		lsb = twl6030_gpadc_read(gpadc, TWL6025_GPCH0_LSB);
	} else {
		...
		u8 reg_base, u16 channels, struct twl6030_gpadc_request *req)
{
	...
	/* TWL6030 has 10bit ADC whilst TWL6025 has 12bit ADC */
	if (twl_features() & TWL6025_SUBCLASS)
		bits = 12;
	else
		bits = 10;
	...
	for (i = 0; i < TWL6030_GPADC_MAX_CHANNELS; i++) {
		if (channels & (1<<i)) {
			reg = reg_base + 2 * i;
			raw_code = twl6030_gpadc_channel_raw_read(gpadc, reg, i);
...
static void twl6030_gpadc_work(struct work_struct *ws)
{
	...
	for (i = 0; i < TWL6030_GPADC_NUM_METHODS; i++) {
		...
		/* Disable GPADC now we are finished with it */
		twl_i2c_read_u8(TWL6030_MODULE_ID1, &rd_reg, PWDNSTATUS2);
		rd_reg = (rd_reg & 0x30) >> 2;
		rd_reg = rd_reg | GPADCR;
	...
	const struct twl6030_gpadc_conversion_method *method)
{
	...
	for (i = 0; i < TWL6030_GPADC_MAX_CHANNELS; i++) {
		...
		twl6030_gpadc_write(the_gpadc, TWL6025_GPSELECT_ISB, i);
...
static int __devinit twl6030_gpadc_probe(struct platform_device *pdev)
{
	...
	ret = request_irq(platform_get_irq(pdev, 0), twl6030_gpadc_irq_handler,
			0, "twl6030_gpadcr", &gpadc->requests[TWL6030_GPADC_RT]);
	ret = request_irq(platform_get_irq(pdev, 1), twl6030_gpadc_irq_handler,
		0, "twl6030_gpadcs", &gpadc->requests[TWL6030_GPADC_SW2]);
	...
	for (index = 0; index < TWL6030_GPADC_MAX_CHANNELS; index++) {
		...
		/* TWL6025 ideal codes will be 4 times larger due to extra 2 bits */
		if (0 & twl_features() & TWL6025_SUBCLASS) {
			u8 l_delta1, l_delta2, h_delta2;
			u8 l_scalar1, l_scalar2;
			u8 sign;

			if (~scalar_bit_map & (1 << index)) {
				twl_i2c_read_u8(TWL6030_MODULE_ID2,
					&l_scalar1,
					twl6025_trim[index].delta1_addr);
				twl_i2c_read_u8(TWL6030_MODULE_ID2,
					&l_scalar2,
					twl6025_trim[index].delta2_addr);

				l_scalar1 &= twl6025_trim[index].delta1_mask;
				sign = l_scalar1 & 1;
				...
				l_scalar2 &= twl6025_trim[index].delta2_mask;
				sign = l_scalar2 & 1;
			...
			twl_i2c_read_u8(TWL6030_MODULE_ID2, &l_delta1,
					TWL6025_GPADC_TRIM5);
			twl_i2c_read_u8(TWL6030_MODULE_ID2, &l_delta2,
					TWL6025_GPADC_TRIM6);
			twl_i2c_read_u8(TWL6030_MODULE_ID2, &h_delta2,
					TWL6025_GPADC_TRIM19);
			...
			ideal_code1 = twl6025_ideal[index].code1 * 4;
			ideal_code2 = twl6025_ideal[index].code2 * 4;
		} else {
			s8 l_delta_error1, l_delta_error2;

			twl_i2c_read_u8(TWL6030_MODULE_ID2, &l_delta_error1,
					twl6030_trim_addr[index]);
			twl_i2c_read_u8(TWL6030_MODULE_ID2, &l_delta_error2,
					(twl6030_trim_addr[index] + 1));
			
			l_delta_error1 = ((s8)(delta_error1 << 1) >> 1);
			l_delta_error2 = ((s8)(delta_error2 << 1) >> 1);

			delta_error1 = l_delta_error1;
			delta_error2 = l_delta_error2;
		...
		gain_error_1 = ((delta_error2 + scalar_delta2) -
				(delta_error1 - scalar_delta1)) * SCALE /
