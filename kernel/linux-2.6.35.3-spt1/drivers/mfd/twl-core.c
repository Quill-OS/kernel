...
#if defined(CONFIG_TWL6030_VBUS_DRIVER) || defined(CONFIG_TWL6030_VBUS_DRIVER_MODULE)
#define twl_has_vbus()	true
#else
#define twl_has_vbus()	false
#endif
...
add_children(struct twl4030_platform_data *pdata, unsigned long features)
{
	...
	if (twl_has_usb() && pdata->usb && twl_class_is_6030()) {
		...
		if (twl_has_regulator() && child)
			...
			if (features & TWL6025_SUBCLASS)
				child = add_regulator(TWL6025_REG_LDOUSB, pdata->ldousb);
			else
	...
	if (twl_has_vbus() && pdata->vbus && twl_class_is_6030()) {
		child = add_child(0, "twl6030_vbus",
				pdata->vbus, sizeof(*pdata->vbus),
				true,
		pdata->irq_base + USB_PRES_INTR_OFFSET, 0);
...
/* 2010/01/17 FY11 : Irq offset 0 -> PWR_INTR_OFFSET. */
				NULL, 0, true, pdata->irq_base + PWR_INTR_OFFSET, 0);
		if (IS_ERR(child))
			return PTR_ERR(child);
	}
