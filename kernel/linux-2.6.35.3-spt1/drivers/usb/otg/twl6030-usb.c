...
static int twl6030_phy_init(struct otg_transceiver *x)
{
	...
	if(pdata->board_control_phy_power)
		pdata->board_control_phy_power(dev, 1);
	else
...
static void twl6030_phy_shutdown(struct otg_transceiver *x)
{
	...
	if(pdata->board_control_phy_power)
		pdata->board_control_phy_power(dev, 0);
	else
...
static int twl6030_usb_ldo_init(struct twl6030_usb *twl)
{
	...
	if (pdata->board_control_phy_power)
		...
		if (twl_features() & TWL6025_SUBCLASS) {
			/* Program the VUSB_CFG_STATE register to ON */
			twl6030_writeb(twl, TWL_MODULE_PM_RECEIVER, 0x01,
						VUSB_CFG_STATE);
		} else {
			/* Program the VUSB_CFG_STATE register to ON on
			 * all groups. */
