...
#define USB_CHARGER_DETECT_TIME 5000
//#undef pr_debug
...
static int detect_charger_complete_flg = 0;
...
extern int twl6030_set_usb_in_current(int currentmA);
#endif

#ifdef CONFIG_REGULATOR_WM831X
extern int sub_cpu_set_power_source(int source);
extern void set_ac_usb_detect_flag(int flag_info);
int set_usb_charge_current(int current_mA);
#endif

static char set_configuration_flag = 0;
...
#ifdef CONFIG_REGULATOR_WM831X
void set_detection_complete_flag(int set_flag)
{
	set_ac_usb_detect_flag(set_flag);
}
#endif

static int set_power_source(int source)
{
#ifdef CONFIG_REGULATOR_TWL4030 
	return twl6030_set_power_source(source);
#endif

#ifdef CONFIG_REGULATOR_WM831X
	return sub_cpu_set_power_source(source);
#endif
}

#ifdef CONFIG_REGULATOR_TWL4030 
static int set_usb_in_current(int currentmA)
{
	return twl6030_set_usb_in_current(currentmA);
}

static void set_charge_current_0mA(struct work_struct *ignored)
{
	set_usb_in_current(0);
}

static void set_charge_current_100mA(struct work_struct *ignored)
{
	set_usb_in_current(100);
}

static void set_charge_current_500mA(struct work_struct *ignored)
{
	set_usb_in_current(500);
}

static void set_charge_current_1000mA(struct work_struct *ignored)
{
	set_usb_in_current(1000);
}
#endif
...
static void check_usb_charger(void)
{
	if( !detect_charger_complete_flg ){
		if (!wake_lock_active(&detect_usb_charger_wake_lock)
			&& !detect_charger_complete_flg ){
...
static void check_charger_fn(unsigned long _udc)
{
	...
	if (is_usb_charger())
		...
		if (check_charger_timer_exec_counter > 20) {
...
static void check_charger_debounce_fn(unsigned long _udc)
{
	if( !detect_charger_complete_flg ){
		if (is_usb_charger()) {
			printk(KERN_INFO"%s: it is DCP or CDP\n", __func__);
			
			usb_charger_handler();
			
			detect_charger_complete_flg = 1;
#ifdef CONFIG_REGULATOR_WM831X
			set_detection_complete_flag(detect_charger_complete_flg);
#endif
			if (wake_lock_active(&detect_usb_charger_wake_lock)){
...
static void check_long_duration_plug_in_fn(unsigned long _udc)
{
	...
	if( !detect_charger_complete_flg ){
		...
		if (((fsl_usb_battery_charger == CHARGER_SUSPEND_HOST) \
			...
			set_detection_complete_flag(detect_charger_complete_flg);
#endif
			set_power_source(POWER_SUPPLY_TYPE_USB);
...
static void usb_charger_handler(void)
{
	...
	if ((temp & PORTSCX_LINE_STATUS_BITS) == PORTSCX_LINE_STATUS_UNDEF) {
		...
		//Set power source
		set_power_source(POWER_SUPPLY_TYPE_MAINS);
...
static void start_otg_phy(void)
{
	u32 otg_phy_ctrl_0;
	u32 otg_portsc;
	
	otg_portsc = fsl_readl(&dr_regs->portsc1);
	fsl_writel(otg_portsc&(~PORTSC_PHCD), &dr_regs->portsc1);

	otg_phy_ctrl_0 = fsl_readl(&dr_regs->phyctrl0);
	otg_phy_ctrl_0 &= ~(1<<11); /* Release PHY reset */
	otg_phy_ctrl_0 |= (1<<12); /* Release low_power mode (start calibration) */
	fsl_writel(otg_phy_ctrl_0, &dr_regs->phyctrl0);
}

static void stop_otg_phy(void)
{
	u32 otg_phy_ctrl_0;
	u32 otg_portsc;
	
	otg_phy_ctrl_0 = fsl_readl(&dr_regs->phyctrl0);
	otg_phy_ctrl_0 &= ~(1<<12);     /* Force low_power mode */
	otg_phy_ctrl_0 |= (1<<11);      /* assert PHY reset */
	fsl_writel(otg_phy_ctrl_0, &dr_regs->phyctrl0);
...
static int fsl_ep_dequeue(struct usb_ep *_ep, struct usb_request *_req)
{
	...
	if (!_ep || !_req || !(fsl_readl(&dr_regs->usbcmd) & USB_CMD_RUN_STOP))
		return -EINVAL;
...
static void fsl_ep_fifo_flush(struct usb_ep *_ep)
{
	...
	if (!(fsl_readl(&dr_regs->usbcmd) & USB_CMD_RUN_STOP))
		return;
	
#define FSL_UDC_FLUSH_TIMEOUT 100
...
static struct usb_gadget_ops fsl_gadget_ops = {
	...
	.wakeup = fsl_wakeup,
/*	.set_selfpowered = fsl_set_selfpowered,	*/ /* Always bus-powered */
		...
		u16 index, u16 length)
{
	...
	if ((request_type & USB_RECIP_MASK) == USB_RECIP_DEVICE) {
		/* Get device status */
		//tmp = 1 << USB_DEVICE_SELF_POWERED;	/* the device being declared as bus-powered */
		tmp &= ~(1 << USB_DEVICE_SELF_POWERED); /* the device being declared as bus-powered */
...
#if 1 /* E_BOOK *//* bugfix 2011/05/09 */
		} else {
		  if ( real_len >= actual ) {
		    if ( actual > EP_MAX_LENGTH_TRANSFER ) {
		      actual = EP_MAX_LENGTH_TRANSFER;
		    }
		    real_len -= actual;
		  } else {
		    actual = real_len;
...
static void reset_irq(struct fsl_udc *udc)
{
	...
	/* Clear set_configuration_flag */
	set_configuration_flag = 0;
...
{
	/* postpone close clock due to detect long duration plug in event */
	msleep(USB_CHARGER_DETECT_TIME);
...
bool try_wake_up_udc(struct fsl_udc *udc)
{
	...
	if (irq_src & OTGSC_B_SESSION_VALID_IRQ_STS) {
		...
		if (irq_src & OTGSC_B_SESSION_VALID) {
			...
			//Set power source
			set_power_source(POWER_SUPPLY_TYPE_BATTERY);
...
static int udc_suspend(struct fsl_udc *udc)
{
	...
	if (udc_controller->gadget.dev.parent->power.status
		...
		if (!udc_can_wakeup_system()){
			printk("udc_can_wakeup_system disabled\n");
			dr_wake_up_enable(udc, false);
		}
		else{
			printk("udc_can_wakeup_system enabled\n");
			dr_wake_up_enable(udc, true);
		}
	}
...
static int fsl_udc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (udc_controller->gadget.dev.driver == 0){
		return 0;
	}
