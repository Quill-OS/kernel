...
static struct usb_string strings_dev[] = {
	...
	[STRING_MANUFACTURER_IDX].s = "Sony",
	[STRING_PRODUCT_IDX].s = "Sony Digital Book Reader",
	[STRING_SERIAL_IDX].s = _usb_string_descriptor_iSerial,
	{  }			/* end of list */
};
...
static int android_probe(struct platform_device *pdev)
{
	...
	if (pdata->product_id) {
		...
		device_desc.idProduct = __constant_cpu_to_le16(pdata->product_id);
	}
	if (pdata->version) {
	...
	if (pdata->serial_number) {
		...
		index = rawdata_index(RAWLABEL_ID, &size);
		if (size == 0) {
			dev_warn(&pdev->dev, "No serial found\n");
		} else {
			char	serial[sizeof _usb_string_descriptor_iSerial];

			memset(serial, 0, sizeof serial);
			if (rawdata_read(index, STRING_SERIAL_EMMC_ID_OFFSET, serial, sizeof serial - 1) < 0) {
				dev_warn(&pdev->dev, "Can't read serial\n");
			} else {
				for (index = 0; index < (sizeof serial - 1); index++) {
					if (isdigit(serial[index]) == 0) {
						dev_warn(&pdev->dev, "No serial setting: %s(default)\n", _usb_string_descriptor_iSerial);
						break;
					}
				}
				if (index == strlen(_usb_string_descriptor_iSerial)) {
					strncpy(_usb_string_descriptor_iSerial, serial, sizeof _usb_string_descriptor_iSerial);
				}
			}
		}
	}

	wake_lock_init(&dev->wake_lock, WAKE_LOCK_SUSPEND, "android_usb");
...
extern int f_adb_init(void);
extern int f_ums_init(void);
extern int f_rndis_init(void);
extern void f_ums_exit(void);
extern void f_rndis_exit(void);
...
static int __init init(void)
{
	...
	ret = f_adb_init();
	if (ret) {
		printk("Failed to init adb\n");
		return ret;
	}

	ret = f_ums_init();
	if (ret) {
		printk("Failed to init ums\n");
		return ret;
	}
