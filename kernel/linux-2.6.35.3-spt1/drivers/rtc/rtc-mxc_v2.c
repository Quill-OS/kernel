...
#define MXC_RTC "rtc1"
#define ERR_PF() printk(KERN_ERR"Error: %s %d\n", __func__, __LINE__);
...
static char state_flag;

static DEFINE_SPINLOCK(rtc_lock);

extern int get_charging_info(void);
extern int get_battery_capacity_info(void);

static int set_rtc_alarm(unsigned int rtc_countdown)
{
    struct rtc_wkalrm alarm;
    struct rtc_time now, future;
    unsigned long t;
	struct rtc_device *rtc = rtc_class_open(MXC_RTC);
    ...
    rtc_tm_to_time(&now, &t);
    t += rtc_countdown;
    rtc_time_to_tm(t, &future);
	alarm.time = future;
	alarm.enabled = 1;
	...
	printk(KERN_NOTICE "%s alarms after %d sec\n", rtc->name, rtc_countdown);
	return 0;
}

static int stop_rtc_alarm(void)
{
    struct rtc_wkalrm alarm;
    struct rtc_time now, future;
    unsigned long t;
...
static struct delayed_work mxc_rtc_delay_worker;

void mxc_rtc_delay_work(struct work_struct *work)
{
#ifdef CONFIG_REGULATOR_WM831X
	if( (get_charging_info() == POWER_SUPPLY_STATUS_CHARGING)
		&& (get_battery_capacity_info() <= 20) )
	{
		set_rtc_alarm(180); //3min
	}
	else if (state_flag == EARLY_SUSPEND_MODE_NORMAL)
	{
		set_rtc_alarm(3600); // 60min
	}
#endif
	
	
#ifdef CONFIG_REGULATOR_TWL4030 
	if( state_flag == EARLY_SUSPEND_MODE_NORMAL )
	{
...
static void rtc_mxc_early_suspend(struct early_suspend *h)
{
#ifdef CONFIG_REGULATOR_WM831X
	if( (get_charging_info() == POWER_SUPPLY_STATUS_CHARGING)
		&& (get_battery_capacity_info() <= 20) )
	...
	else if (h->pm_mode == EARLY_SUSPEND_MODE_NORMAL)
	{
		set_rtc_alarm(3600); // 60min
	}
#endif
	...
	state_flag = h->pm_mode;
	
}

static void rtc_mxc_late_resume(struct early_suspend *h)
{
	stop_rtc_alarm();
	//clr_rtc_alarm();
...
static struct early_suspend rtc_mxc_earlysuspend = {
	.level = EARLY_SUSPEND_LEVEL_DISABLE_FB,
	.suspend = rtc_mxc_early_suspend,
	.resume = rtc_mxc_late_resume,
};
#endif
...
static irqreturn_t mxc_rtc_interrupt(int irq, void *dev_id)
{
	...
	schedule_delayed_work(&mxc_rtc_delay_worker, msecs_to_jiffies(50));
...
static int mxc_rtc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (device_may_wakeup(&pdev->dev)) {
		//printk(KERN_NOTICE"%s enable_irq_wake\n", __func__);
...
{
	INIT_DELAYED_WORK_DEFERRABLE(&mxc_rtc_delay_worker, mxc_rtc_delay_work);

#ifdef CONFIG_EARLYSUSPEND
	register_early_suspend(&rtc_mxc_earlysuspend);
#endif
...
{
#ifdef CONFIG_EARLYSUSPEND
	unregister_early_suspend(&rtc_mxc_earlysuspend);
#endif
