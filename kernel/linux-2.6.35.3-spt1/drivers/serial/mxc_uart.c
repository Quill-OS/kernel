...
#if 1 /* E_BOOK *//* for Serial Switcher 2011/2/2 */
#include <linux/irq.h>
#endif
#include <asm/irq.h>
#include <asm/dma.h>
...
#include <mach/gpio.h>
#endif
#if 1 /* E_BOOK *//* for Lowpower 2011/2/24 */
...
#if 1 /* E_BOOK */
/* for Serial swither 2011/1/5 */
/*-----------------------------------------------
  Serial Switcher
  ----------------------------------------------*/

static struct mxc_ssw_link mxc_ssw_link[MXC_SSW_MAXDEV];

/* 
   commons
*/
/* interrupt for RTS */
static irqreturn_t mxc_ssw_rts_intr(int irq, void *dev_id)
{
  uart_mxc_port *umxc = dev_id;

#if 1 /* E_BOOK *//* bugfix: do only RTS is low, 2011/04/14 */
  if (mxc_ssw_link[umxc->slotno].ssw_mode == MXC_SSW_MODE_INPUT) {
    gpio_set_value(umxc->cts_gpio,0); /* CTS set to LOW */
  } else {
    if (gpio_get_value(umxc->rts_gpio)) {
      set_irq_type(irq, IRQ_TYPE_EDGE_FALLING);
      gpio_set_value(umxc->cts_gpio,1); /* CTS set to HIGH */
    } else {
...
/* init ssw */
int mxc_ssw_init(uart_mxc_port *umxc)
{
  int rc;
  int irq;
  if ( umxc->cts_gpio  > 0 ) {
    gpio_request(umxc->cts_gpio, "irtp");
    gpio_direction_output(umxc->cts_gpio, 0); /* change initial value for leak signal at cold boot 2011/08/23 */
  } 
  if ( umxc->rts_gpio  > 0 ) {
    gpio_request(umxc->rts_gpio, "irtp");
    gpio_direction_input(umxc->rts_gpio);
    irq = gpio_to_irq(umxc->rts_gpio);
#if 1 /* E_BOOK *//* bugfix: do only RTS is low, 2011/04/14 */
    set_irq_type(irq, IRQ_TYPE_EDGE_FALLING);
#else
    if (gpio_get_value(umxc->rts_gpio)) {
...
#endif
    /* gpio */
    rc = request_irq(irq, mxc_ssw_rts_intr, 0,"mxcintswuart", umxc); 
  }
  return 0;
}


int mxc_ssw_serialmode(int port ) 
{
  if ( port < 0  || port >=MXC_SSW_MAXDEV ) {
    return -EINVAL;
  }
  if ( mxc_ssw_link[port].serialdev == NULL ) {  
    return -ENODEV;
  }
#if 0 /* E_BOOK *//* for DEBUG */
  printk("SSW: set serial mode(%d)\n",port);
#endif
  mxc_ssw_link[port].ssw_mode = MXC_SSW_MODE_SERIAL;
  return 0;
}

int mxc_ssw_inputdevmode(int port ) 
{
  int flags;
  int irq;
  if ( port < 0  || port >= MXC_SSW_MAXDEV ) {
  ...
  if ( mxc_ssw_link[port].inputdev == NULL ) {  
    return -ENODEV;
  }
  local_irq_save(flags);
#if 0 /* E_BOOK *//* for DEBUG */
  printk("SSW: set input mode(%d)\n",port);
#endif
  mxc_ssw_link[port].ssw_mode = MXC_SSW_MODE_INPUT;
  irq = gpio_to_irq(mxc_ssw_link[port].umxc->rts_gpio);
  set_irq_type(irq, IRQ_TYPE_EDGE_FALLING);
  local_irq_restore(flags);
  return 0;
}

int mxc_ssw_is_serialmode(int port ) 
{
  if ( port < 0  || port >=MXC_SSW_MAXDEV ) {
    return -EINVAL;
  }
  ...
  if ( mxc_ssw_link[port].ssw_mode == MXC_SSW_MODE_SERIAL) {
    return 1;
  }
  return 0;
}
int mxc_ssw_is_inputdevmode(int port ) 
{
  if ( port < 0  || port >=MXC_SSW_MAXDEV ) {
    return -EINVAL;
  }
  ...
  if ( mxc_ssw_link[port].ssw_mode == MXC_SSW_MODE_INPUT) {
    return 1;
  }
  return 0;
}

void mxc_ssw_insert_char(uart_mxc_port *umxc,unsigned int ch,unsigned int flag)
{
  struct mxc_ssw_recept *recp;
#if 0 /* E_BOOK *//* for DEBUG 2011/1/28 */
  //dump_stack();
  printk("mxc_ssw_insert_char %x\n",ch);
#endif
  recp = mxc_ssw_link[umxc->slotno].inputdev;
  if ( recp->recvbuf_index >= MXC_SSW_RECVBUFSIZE ) {
    return;
  }
  recp->recvbuf[recp->recvbuf_index] = ch;
  recp->recvbuf_index++;
}

static int mxc_ssw_rts_edge=0;
void mxc_ssw_clear_rts_edge()
{
  unsigned long flag;
  local_irq_save(flag);
  mxc_ssw_rts_edge=0;
...
int mxc_ssw_get_rts_edge()
{
  return mxc_ssw_rts_edge;
}
/* 
   for Receptor side function 
*/
int mxc_ssw_recept_register( struct mxc_ssw_recept *input , void *arg, int port) 
{
  if ( port < 0  || port >=MXC_SSW_MAXDEV ) {
    return -EINVAL;
  }
  if ( mxc_ssw_link[port].inputdev != 0 ) {
    return -EBUSY;
  }
  /*
  if ( mxc_ssw_link[port].serialdev == NULL ) {
  ...
  */
  mxc_ssw_link[port].inputdev = input;
  mxc_ssw_link[port].inputarg = arg;
  mxc_ssw_link[port].flag |= 0x0001;
  input->recvbuf_index=0;
  if ( mxc_ssw_link[port].serialdev != NULL ) {
    clk_enable(mxc_ssw_link[port].umxc->clk);
    mxc_ssw_link[port].umxc->port.ops->startup(&mxc_ssw_link[port].umxc->port);
    mxc_ssw_link[port].umxc->port.ops->set_mctrl(&mxc_ssw_link[port].umxc->port,TIOCM_RTS);/* change initial value for leak signal at cold boot 2011/08/23 */
    if (input->connect) {
      input->connect(arg);
    }
  }
...
int mxc_ssw_recept_unregister(int port) 
{
  if ( port < 0  || port >=MXC_SSW_MAXDEV ) {
    return -EINVAL;
  }
  if ( mxc_ssw_link[port].flag == 0  ) {
    return 0;
  }
  if ( mxc_ssw_link[port].flag & 0x1  ) {
    if ( mxc_ssw_link[port].inputdev->disconnect ) {
      mxc_ssw_link[port].inputdev->disconnect(mxc_ssw_link[port].inputarg);
    }
    mxc_ssw_link[port].inputdev = NULL;
    mxc_ssw_link[port].flag &= 0xff00;
    return 0;
  }
  if ( mxc_ssw_link[port].flag & 0x100  ) {
    mxc_ssw_serialmode(port);
  }
  return 0;
}

/* setup options */
int mxc_ssw_set_options(int slot, struct console *co, int baud, int parity, int bits, int flow)
{
  int rc;
  rc = uart_set_options(&mxc_ssw_link[slot].umxc->port,co, baud, parity, bits, flow);
  return rc;
}

/* term ios */
int mxc_ssw_set_termios(int slot, struct ktermios *termios, struct ktermios *old)
{
  int rc=0; 

  if ( mxc_ssw_link[slot].ssw_mode != MXC_SSW_MODE_INPUT ) {
    return -EINVAL;
  }
  
  mxc_ssw_link[slot].umxc->port.ops->set_termios(&(mxc_ssw_link[slot].umxc->port),termios,old);

  return rc;
}

int mxc_ssw_set_mctrl(int slot,unsigned int mctrl)
{
  int rc=0; 
  ...
  mxc_ssw_link[slot].umxc->port.ops->set_mctrl(&mxc_ssw_link[slot].umxc->port, mctrl);

#if 0 /* E_BOOK *//* for DEBUG */
  printk("mxc_ssw_set_mctrl:cr2=%x,mctl=%x\n",readl(mxc_ssw_link[slot].umxc->port.membase + MXC_UARTUCR2),mctrl);
#endif
  return rc;
}

int mxc_ssw_get_mctrl(int slot,unsigned int *mctrl)
{
  int rc=0; 
  ...
  *mctrl = mxc_ssw_link[slot].umxc->port.ops->get_mctrl(&mxc_ssw_link[slot].umxc->port);

  return rc;
}

/* send */
int mxc_ssw_send_xchar(int slot,char ch)
{
  int rc=0; 
  uart_mxc_port *umxc;
...
#if 1 /* E_BOOK *//* 2011/2/2 */
  umxc = mxc_ssw_link[slot].umxc;
  writel(ch, umxc->port.membase + MXC_UARTUTXD);  
#else
  mxc_ssw_link[slot].umxc->port.ops->send_xchar(&mxc_ssw_link[slot].umxc->port, ch);
#endif

  return rc;
}
/* send buffer */
int mxc_ssw_send(int slot,char *buf,int size)
{
  int i;
  int rc;
  for ( i = 0 ; i < size; i++) {
    rc = mxc_ssw_send_xchar(slot,buf[i]);
    if ( rc < 0 ) {
      return rc;
    }
    //udelay(10);
...
int mxc_ssw_startup(int slot)
{
  ...
  mxc_ssw_link[slot].umxc->port.ops->startup(&mxc_ssw_link[slot].umxc->port);

  return rc;
}
int mxc_ssw_shutdown(int slot)
{
  int rc=0; 
  ...
  mxc_ssw_link[slot].umxc->port.ops->shutdown(&mxc_ssw_link[slot].umxc->port);

  return rc;
}
void mxc_ssw_enable_irq_wakeup(int slot)
{
  uart_mxc_port *umxc;
  int irq;
  umxc = mxc_ssw_link[slot].umxc;
...
void mxc_ssw_disable_irq_wakeup(int slot)
{
  uart_mxc_port *umxc;
  int irq;
  umxc = mxc_ssw_link[slot].umxc;
...
/*
 for PORT side function 
*/
int mxc_ssw_port_register( struct mxc_ssw_port *port , void *arg, uart_mxc_port *umxc) 
{
  int slot = umxc->slotno;

  printk("SSW:ssw port register %d\n",slot);
  if ( slot < 0  || slot >=MXC_SSW_MAXDEV ) {
    return -EINVAL;
  }
  if ( mxc_ssw_link[slot].serialdev != 0 ) {
    return -EBUSY;
  }

  mxc_ssw_link[slot].serialdev = port;
  mxc_ssw_link[slot].flag |= 0x0100;
  mxc_ssw_link[slot].serialarg = arg;
  mxc_ssw_link[slot].umxc = umxc;
  
  if (mxc_ssw_link[slot].inputdev != NULL ) {
    clk_enable(umxc->clk);
    mxc_ssw_link[slot].umxc->port.ops->startup(&mxc_ssw_link[slot].umxc->port);
    mxc_ssw_link[slot].umxc->port.ops->set_mctrl(&mxc_ssw_link[slot].umxc->port,TIOCM_RTS);/* change initial value for leak signal at cold boot 2011/08/23 */
    if (mxc_ssw_link[slot].inputdev->connect) {
      mxc_ssw_link[slot].inputdev->connect(mxc_ssw_link[slot].inputarg);
    }
  }
...
int mxc_ssw_port_unregister(int port) 
{
  if ( port < 0  || port >=MXC_SSW_MAXDEV ) {
    return -EINVAL;
  }
  ...
  mxc_ssw_link[port].serialdev = NULL;
  mxc_ssw_link[port].flag &= 0xff;
  mxc_ssw_link[port].serialarg = NULL;
  if ( mxc_ssw_link[port].inputdev != NULL) {
    if (mxc_ssw_link[port].inputdev->disconnect) {
      mxc_ssw_link[port].inputdev->disconnect(mxc_ssw_link[port].inputarg);
    }
...
/* up to receive data */
void mxc_ssw_tty_flip_buffer_push(uart_mxc_port * umxc)
{
  int slot = umxc->slotno;
  int i;
  ...
  if ( mxc_ssw_link[slot].ssw_mode == MXC_SSW_MODE_INPUT ) {
#if 0 /* E_BOOK *//* for DEBUG 2011/1/28 */
    printk("mxc_ssw_tty_flip_buffer_push\n");
#endif
    if (mxc_ssw_link[slot].inputdev->recv){
      int size = mxc_ssw_link[slot].inputdev->recvbuf_index;
      if ( size > 0 ) {
	for ( i = 0; i < size ; i++) {
	  mxc_ssw_link[slot].inputdev->recv(mxc_ssw_link[slot].inputarg,
					    mxc_ssw_link[slot].inputdev->recvbuf[i]);
	}
      } else {
	printk("SSW:no recv data\n");
      }
    }
    mxc_ssw_link[slot].inputdev->recvbuf_index=0;
  } else {
    tty_flip_buffer_push(umxc->port.state->port.tty);
  }
}
void mxc_ssw_port_close(int slot)
{
  struct pad_desc uart2_txd = MX50_PAD_UART2_TXD__GPIO_6_10;
  struct pad_desc uart2_rxd = MX50_PAD_UART2_RXD__GPIO_6_11;
 
  //printk("mxc_ssw_port_close\n");
  gpio_request(5*32+10, "irtp");
  gpio_request(5*32+11, "irtp");
  //gpio_direction_input(5*32+10);
  gpio_direction_output(5*32+10,0);
  gpio_direction_input(5*32+11);
  mxc_iomux_v3_setup_pad(&uart2_txd);
  mxc_iomux_v3_setup_pad(&uart2_rxd);
  //gpio_set_value(5*32+10,0);
  gpio_free(5*32+10);
  gpio_free(5*32+11);
}
void mxc_ssw_port_open(int slot)
{
  struct pad_desc uart2_txd =	MX50_PAD_UART2_TXD__UART2_TXD;
  struct pad_desc uart2_rxd =	MX50_PAD_UART2_RXD__UART2_RXD;
  uart_mxc_port *umxc =  mxc_ssw_link[slot].umxc;
  unsigned int cr1,cr2;

  //printk("mxc_ssw_port_open\n");
  //gpio_set_value(5*32+10,1);
  mxc_iomux_v3_setup_pad(&uart2_txd);
  mxc_iomux_v3_setup_pad(&uart2_rxd);
  cr1 = readl(umxc->port.membase + MXC_UARTUCR1);
  cr2 = readl(umxc->port.membase + MXC_UARTUCR2);
}
#endif /* E_BOOK */
...
#if 0 /* E_BOOK *//* for DEBUG 2011/1/19 */
	printk("mxcuart_enable_ms:port=%p\n",port);
#endif
	/*
...
#if 1 /* E_BOOK */
		if ( (umxc->sswmode == 1) && 
		     (mxc_ssw_link[umxc->slotno].ssw_mode == MXC_SSW_MODE_INPUT)) {
		  mxc_ssw_insert_char(umxc,ch,flag);
		} else {
...
#if 1 /* E_BOOK */
	/* for Serial switcher 2011/1/5 */
	if ( umxc->sswmode == 1 ) {
	  mxc_ssw_tty_flip_buffer_push(umxc);
	} else {
	  tty_flip_buffer_push(umxc->port.state->port.tty);
	}
#else
...
#if 0 /* E_BOOK *//* for DEBUG 2011/2/2 */
	if ( umxc->sswmode ==1 ) {
	  printk("mxcuart_tx_chars:fifosize=%d,tx_threshold=%d,tail=%d\n",
		 umxc->port.fifosize,umxc->tx_threshold,xmit->tail);
	}
#endif
...
#if 0 /* E_BOOK *//* for DEBUG */
		if ( umxc->sswmode == 1 ) {
		  printk("mxuart_int:sr1=%08x,sr2=%08x\n",sr1,sr2);
		  printk("mxuart_int:(read)cr1=%08x,cr2=%08x\n",
		  	 readl(umxc->port.membase + MXC_UARTUCR1),
		  	 readl(umxc->port.membase + MXC_UARTUCR2));
		  //printk("mxuart_int:(read)sr1=%08x,sr2=%08x\n",
		  //     readl(umxc->port.membase + MXC_UARTUSR1),
		  //     readl(umxc->port.membase + MXC_UARTUSR2));
		}
#endif
		/*
...
#if 1 /* E_BOOK *//* for Serial Switcher 2011/2/2 */
	if ( (umxc->sswmode == 1 ) && (umxc->rts_gpio >= 0 ) ) {
	  if ( gpio_get_value(umxc->rts_gpio) == 0 ) {
		result |= TIOCM_CTS;
	  }
	  return result;
	}
...
#if 0 /* E_BOOK *//* for DEBUG */
	if ( umxc->sswmode == 1 ) {
	  printk("mxcuart_set_mctrl: port = %p,mctrl=%x\n",port,mctrl);
	  printk("mxcuart_set_mctrl:sr1=%08x,sr2=%08x\n",
		 readl(umxc->port.membase + MXC_UARTUSR1),
		 readl(umxc->port.membase + MXC_UARTUSR2));
	  printk("mxcuart_set_mctrl:cr1=%08x,cr2=%08x,cr3=%08x\n",
		 readl(umxc->port.membase + MXC_UARTUCR1),
		 readl(umxc->port.membase + MXC_UARTUCR2),
		 readl(umxc->port.membase + MXC_UARTUCR3));
	  printk("mxcuart_set_mctrl:uts=%08x\n",
		 readl(umxc->port.membase + MXC_UARTUTS));
	}
#endif

#if 1 /* E_BOOK *//* for Serial Switcher 2011/2/2 */
	if ( (umxc->sswmode == 1 ) && (umxc->cts_gpio >= 0 ) ) {
	  if (mctrl & TIOCM_RTS ) {
	    gpio_set_value(umxc->cts_gpio,0); /* CTS set to LOW */
	  } else {
	    gpio_set_value(umxc->cts_gpio,1); /* CTS set to HIGH */
...
#if 0 /* E_BOOK *//* for Serail Switcher 2011/1/28 */
	if ( umxc->sswmode == 1 ) {
	  cr2 |= ( 0x03 << 9 )/* |MXC_UARTUCR2_RTSEN*/; /* RTS any edge interrupt */
	}
#endif
...
#if 0 /* E_BOOK *//* for DEBUG 2011/1/18 */
	if ( umxc->sswmode == 1 ) {
	  printk("mxcuart_startup: return cr1 = %x\n",cr1);
	}
#endif
	return 0;
}
...
#if 0 /* E_BOOK *//* for DEBUG 2011/1/18 */
	if (umxc->sswmode == 1 ) {
	  printk("set termios: baud = %d, max=%d\n",baud,per_clk / 16);
	}
#endif
...
#if 1 /* E_BOOK */
	/* for Serial Switcher  2011/1/18 */
	if ((umxc->sswmode == 1) ) {
	  umxc->hardware_flow = 0;
	} else {
...
static int mxcuart_request_port(struct uart_port *port)
{
	...
	//printk("mxcuart_request_port: port = %p\n",port);
...
{
	unsigned long flags;
#if 1 /* E_BOOK *//* for serial switch 2011/1/12 */
	uart_mxc_port *umxc = (uart_mxc_port *)port;
#endif
...
#if 1 /* E_BOOK *//* for serial switch 2011/1/12 */
	if (umxc->sswmode) {
	  if (mxc_ssw_link[umxc->slotno].ssw_mode == MXC_SSW_MODE_SERIAL) {
...
#if 1 /* E_BOOK */
/* for switcher serial 2011/1/5 */
static int mxcuart_ssw_suspend(struct platform_device *pdev, pm_message_t state)
{
  int id = pdev->id;
  /* call input suspend */
  if ( mxc_ssw_link[id].inputdev != NULL ) {
    if (mxc_ssw_link[id].inputdev->suspend) {
      mxc_ssw_link[id].inputdev->suspend(mxc_ssw_link[id].inputarg);
    }
  }
  /* call serial suspend */
  mxcuart_suspend(pdev,state);

  return 0;
}
static int mxcuart_ssw_resume(struct platform_device *pdev)
{
  int id = pdev->id;
  ...
  /* call input resume */
  if ( mxc_ssw_link[id].inputdev != NULL ) {
    if (mxc_ssw_link[id].inputdev->resume) {
      mxc_ssw_link[id].inputdev->resume(mxc_ssw_link[id].inputarg);
    }
  }
  return 0;
}
...
#if 1 /* E_BOOK */
/* for switcher serial 2011/1/5 */
static struct mxc_ssw_port mxc_ssw_port[] = {
  {
    .disconnect = NULL,
  },
  {
...
/**/
static int mxcuart_ssw_probe(struct platform_device *pdev)
{
  int id = pdev->id;
  int ret = 0;

  ret = mxcuart_probe(pdev);
  //printk("SSW:mxcuart_ssw_probe %d,port=%p\n",id,mxc_ports[id]);
  if ( ret == 0 ) {
    mxc_ports[id]->sswmode=1;
    mxc_ports[id]->slotno=id;
    mxc_ports[id]->hardware_flow = 0;
    if ( id == 0 ) {
      mxc_ports[id]->cts_gpio = 5*32 + 8; /* CTS GPIO set to GPIO 6_8 */
      mxc_ports[id]->rts_gpio = 5*32 + 9; /* RTS GPIO set to GPIO 6_9 */
    } else if ( id == 1 ) {
      mxc_ports[id]->cts_gpio = 5*32 + 12; /* CTS GPIO set to GPIO 6_12 */
      mxc_ports[id]->rts_gpio = 5*32 + 13; /* RTS GPIO set to GPIO 6_13 */
    } else {
      /* other set -1 */
      mxc_ports[id]->cts_gpio = -1; 
      mxc_ports[id]->rts_gpio = -1; 
    }
    mxc_ssw_init(mxc_ports[id]);
    ret = mxc_ssw_port_register(&mxc_ssw_port[id],NULL,mxc_ports[id]);
  }
  return ret;
}
static int mxcuart_ssw_remove(struct platform_device *pdev)
{
  int id = pdev->id;
  int ret = 0;
  ret = mxc_ssw_port_unregister(id);
  if (ret == 0) {
    mxcuart_remove(pdev);
  }
  return ret;
...
#if 1 /* E_BOOK */
/* for switcher serial 2011/1/5 */
static struct platform_driver mxcswuart_driver = {
	.driver = {
		   .name = "mxcintswuart",
		   },
	.probe = mxcuart_ssw_probe,
	.remove = mxcuart_ssw_remove,
	.suspend = mxcuart_ssw_suspend,
	.resume = mxcuart_ssw_resume,
};
#endif
/*!
...
static int __init mxcuart_init(void)
{
	...
	if (ret == 0) {
		/* Register the device driver structure. */
	  	printk(KERN_INFO "mxcuart register\n");
		ret = platform_driver_register(&mxcuart_driver);
		if (ret != 0) {
			uart_unregister_driver(&mxc_reg);
		}
		//printk(KERN_INFO "mxcuart register return\n");
#if 1 /* E_BOOK */
		/* for switcher serial 2011/1/5 */
		/* Register the device driver structure. */
		printk(KERN_INFO "mxcswuart register\n");
		ret = platform_driver_register(&mxcswuart_driver);
		if (ret != 0) {
...
{
	platform_driver_unregister(&mxcuart_driver);
	platform_driver_unregister(&mxcswuart_driver);
	uart_unregister_driver(&mxc_reg);
}
