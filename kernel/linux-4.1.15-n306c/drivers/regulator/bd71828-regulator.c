...
struct platform_device *g_bd71828_data;
...
int set_bd71828_Wifi_1V1(int status)
{
	int res = 0;
	struct regmap *regmap = dev_get_regmap(g_bd71828_data->dev.parent, NULL);
	if(0x67 == gptHWCFG->m_val.bPCB ){ // E60K24
		if( 1 == status) {
			printk("[%s_%d] Set LDO4 1.1 V \n",__FUNCTION__,__LINE__);
			regmap_write(regmap, BD71828_REG_LDO4_EN, 0x0E);
			//regmap_write(regmap, BD71828_REG_LDO4_VOLT, 0x06); // 1.1v
			res = 1 ;
		}
		else if ( 0 == status) {
			printk("[%s_%d] Close LDO4  \n",__FUNCTION__,__LINE__);
			regmap_write(regmap, BD71828_REG_LDO4_EN, 0x00);
			res =1 ;
		}
		else{
			printk("[%s_%d] Unknow status %d \n",__FUNCTION__,__LINE__,status);
		}
	}
	return res;
}
EXPORT_SYMBOL(set_bd71828_Wifi_1V1);

static int bd71828_regulator_probe(struct platform_device *pdev)
{
	int i, j, ret;
	...
	rdev = devm_kzalloc(&pdev->dev, ARRAY_SIZE(bd71828_rdata) *
	...
	if(gptHWCFG->m_val.bCPU == 10) {
		printk ("[%s-MX6SLL]...turn-off none used\n", __func__);
		//E60K02,E70K14
		regmap_write(config.regmap, BD71828_REG_BUCK6_EN, 0x0);
		regmap_write(config.regmap, BD71828_REG_BUCK7_EN, 0x0);
		regmap_write(config.regmap, BD71828_REG_LDO2_EN, 0x0);
		regmap_write(config.regmap, BD71828_REG_LDO3_EN, 0x0);
		regmap_write(config.regmap, BD71828_REG_LDO6_EN, 0x0);
		if(gptHWCFG->m_val.bPCB == 101) { //E70K14
			regmap_write(config.regmap, BD71828_REG_LDO1_EN, 0x0);
			regmap_write(config.regmap, BD71828_REG_LDO4_EN, 0x0);
		}
		else if(0x67 == gptHWCFG->m_val.bPCB ){ // E60K24
			regmap_write(config.regmap, BD71828_REG_LDO1_EN, 0x0);
			printk(KERN_ERR"[BD71828] Set LD04 1.1 V \n");
			regmap_write(config.regmap, BD71828_REG_LDO4_EN, 0x0E);
			regmap_write(config.regmap, BD71828_REG_LDO4_VOLT, 0x06); // 1.1v
		}
	}
	else if(gptHWCFG->m_val.bCPU == 9) {
		printk ("[%s-MX6SLL]...turn-off none used\n", __func__);
		//E60U22
		regmap_write(config.regmap, BD71828_REG_BUCK2_EN, 0x0);
		regmap_write(config.regmap, BD71828_REG_BUCK6_EN, 0x0);
		regmap_write(config.regmap, BD71828_REG_BUCK7_EN, 0x0);
		regmap_write(config.regmap, BD71828_REG_LDO3_EN, 0x0);
		regmap_write(config.regmap, BD71828_REG_LDO6_EN, 0x0);
		regmap_write(config.regmap, BD71828_REG_LDO7_EN, 0x0);
	}
	else {
		printk ("[%s-%d]...none support %d\n", __func__, __LINE__, gptHWCFG->m_val.bCPU);
	}
...
static int bd71828_regulator_suspend(struct device *dev)
{
	...
	if(gptHWCFG->m_val.bCPU == 10) {
		printk ("[%s-MX6SLL]...\n", __func__);
		// turn off none used bucks & ldos
		regmap_write(regmap, BD71828_REG_BUCK6_EN, 0x0);
		regmap_write(regmap, BD71828_REG_BUCK7_EN, 0x0);
		regmap_write(regmap, BD71828_REG_LDO2_EN, 0x0);
		regmap_write(regmap, BD71828_REG_LDO3_EN, 0x0);
		regmap_write(regmap, BD71828_REG_LDO6_EN, 0x0);
		// TBC
		// regmap_write(regmap, BD71828_REG_LDO7_EN, 0x0);
		if(gptHWCFG->m_val.bPCB == 101) { //E70K10
			regmap_write(regmap, BD71828_REG_BUCK2_EN, 0x0A);       // core1_1v3_arm
			regmap_write(regmap, BD71828_REG_LDO1_EN, 0x0);
			regmap_write(regmap, BD71828_REG_LDO4_EN, 0x0);
		}
		else if(0x67 == gptHWCFG->m_val.bPCB){ // E60K24
			regmap_write(regmap, BD71828_REG_BUCK2_EN, 0x0A);       // core2 1v3_arm
			regmap_write(regmap, BD71828_REG_LDO1_EN, 0x0);
		}

		if (gSleep_Mode_Suspend) {
			// turn off SPD_3v3 & FL_3v3 in hibernation
			regmap_write(regmap, BD71828_REG_LDO5_EN, 0x0A);
		}
		else {
			// keep SPD_3v3 & FL_3v3 in suspend
			regmap_write(regmap, BD71828_REG_LDO5_EN, 0x0E);
		}
	}
	else if(gptHWCFG->m_val.bCPU == 9) {
		// turn off none used bucks & ldos
		printk ("[%s-MX6ULL]...\n", __func__);
		regmap_write(regmap, BD71828_REG_BUCK2_EN, 0x0);
		regmap_write(regmap, BD71828_REG_BUCK6_EN, 0x0);
		regmap_write(regmap, BD71828_REG_BUCK7_EN, 0x0);
		...
		if (gSleep_Mode_Suspend) {
			// turn off SPD_3v3 & FL_3v3 in hibernation
			regmap_write(regmap, BD71828_REG_LDO4_EN, 0x0A);
			regmap_write(regmap, BD71828_REG_LDO5_EN, 0x0A);
		}
		else {
			// keep SPD_3v3 & FL_3v3 in suspend
		...
		printk ("[%s-%d]...bd71828_regulator_suspend none support %d\n", __func__, __LINE__, gptHWCFG->m_val.bCPU);
	}
...
static struct platform_driver bd71828_regulator = {
	...
	.probe = bd71828_regulator_probe,
};

//module_platform_driver(bd71828_regulator);

static int __init bd71828_regulator_init(void)
{
	printk(KERN_INFO "PMU: %s\n", __func__);
	return platform_driver_register(&bd71828_regulator);
}
subsys_initcall_sync(bd71828_regulator_init);

static void __exit bd71828_regulator_exit(void)
{
	platform_driver_unregister(&bd71828_regulator);
}
module_exit(bd71828_regulator_exit);
