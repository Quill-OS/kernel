...
static int bd70528_probe(struct platform_device *pdev)
{
	...
	rtc = devm_rtc_device_register(&pdev->dev, "bd70528", &bd71828_rtc_ops, THIS_MODULE);//devm_rtc_allocate_device(&pdev->dev);
	if (IS_ERR(rtc)) {
	...
	return 0;//devm_rtc_register_device(rtc);
}
...
static int __init bd718x7_rtc_init(void)
{
	printk(KERN_INFO "PMU: %s\n", __func__);
	return platform_driver_register(&bd70528_rtc);
}
subsys_initcall_sync(bd718x7_rtc_init);

static void __exit bd718x7_rtc_exit(void)
{
	platform_driver_unregister(&bd70528_rtc);
}
module_exit(bd718x7_rtc_exit);
