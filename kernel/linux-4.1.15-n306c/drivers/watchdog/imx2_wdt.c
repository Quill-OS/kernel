...
static struct watchdog_device *gtWDOG;
...
unsigned int imx2_get_wdt_timeout(void)
{
	unsigned int L_uiTimeout;

	if (gtWDOG) {
	    L_uiTimeout = gtWDOG->timeout;
	} else {
	    L_uiTimeout = (unsigned int)(-1);
	}

	return L_uiTimeout;
}
EXPORT_SYMBOL(imx2_get_wdt_timeout);
