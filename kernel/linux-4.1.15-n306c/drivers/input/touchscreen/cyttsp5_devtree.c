	...
	if (!rc) {
		...
		if(gptHWCFG) {
			...
			if(0==gptHWCFG->m_val.bUIStyle) {
				...
				if(gptHWCFG->m_val.bPCB== 74 && NTXHWCFG_CFA_FLAG(gptHWCFG->m_val.bEPD_Flags,3) ==1 )// K21 + CFA PANEL
				{
					//pdata->flags |= CY_MT_FLAG_INV_Y ;
				}
				else if(gptHWCFG->m_val.bPCB== 93 && NTXHWCFG_CFA_FLAG(gptHWCFG->m_val.bEPD_Flags,3) ==1 )// K21 + CFA PANEL
				{
					//pdata->flags |= CY_MT_FLAG_INV_Y ;
				}
				else if(gptHWCFG->m_val.bPCB== 74 || gptHWCFG->m_val.bPCB== 73 		// E60K00:74 , E60U10:73,
					|| gptHWCFG->m_val.bPCB == 85 || gptHWCFG->m_val.bPCB == 93   	// T60U00:85 , E60K10:93
					|| gptHWCFG->m_val.bPCB == 103  )  	// E60K20:
				{	
					pdata->flags |= CY_MT_FLAG_INV_Y ;
				}
				...
				// Customer UI .
				if(gptHWCFG->m_val.bPCB== 74 || gptHWCFG->m_val.bPCB== 73 ||
					gptHWCFG->m_val.bPCB== 93 || gptHWCFG->m_val.bPCB== 103 ) // E60K00:74 , E60U10:73 , , E60K10:93 , E60K20:103
				{	
					//pdata->flags |= CY_MT_FLAG_INV_Y ;
				}
				else if(75==gptHWCFG->m_val.bPCB) // E80K02
				...
				else if (gptHWCFG->m_val.bPCB == 85)
				{
					pdata->flags |= CY_MT_FLAG_INV_Y ;
				}
				else
