		...
		int value)
{
	...
	if (sig != CY_IGNORE_VALUE) {
		...
		NTX_TimeStamp_printf(__FUNCTION__,__LINE__,"abs,sig=%d,val=%d",sig,value);
	}
}
		...
		struct cyttsp5_touch *tch, int num_cur_tch)
{
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		if (tch->abs[CY_TCH_E] == CY_EV_LIFTOFF) {
			...
			if(0==gptHWCFG->m_val.bUIStyle || 85==gptHWCFG->m_val.bPCB) {
				...
				NTX_TimeStamp_printf(__FUNCTION__,__LINE__,"touchup,(%d,%d)",tch->abs[CY_TCH_X],tch->abs[CY_TCH_Y]);
			}
		...
		if(0==gptHWCFG->m_val.bUIStyle || 85==gptHWCFG->m_val.bPCB) {
			...
			NTX_TimeStamp_printf(__FUNCTION__,__LINE__,"touchdown,(%d,%d)",
					tch->abs[0],tch->abs[1]);
		}
