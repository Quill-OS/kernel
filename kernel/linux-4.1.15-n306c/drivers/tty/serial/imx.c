...
static bool gi_BT_power_status = false;
...
struct imx_port *bt_port_host;
//struct platform_device *bt_port_host;
void bt_pin_set(bool on)
{
    if (!bt_port_host)
        return;
    
    if (on) { /* set pin to uart */
        pinctrl_select_state(bt_port_host->pinctrl, bt_port_host->pins_default);
    } else { /* set pin to gpio , and pull low */
        pinctrl_select_state(bt_port_host->pinctrl, bt_port_host->pins_sleep);
    }
    
    gi_BT_power_status = on;
}
EXPORT_SYMBOL(bt_pin_set);
...
static int serial_imx_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	/* when suspend bt pinctrl set to gpio mode and pull low */
#if 0
	if (11 == gptHWCFG->m_val.bWifi || 12 == gptHWCFG->m_val.bWifi || 14 == gptHWCFG->m_val.bWifi) {
		if (gi_BT_power_status == true) {
			bt_pin_set(false);
		}
	}
#endif
...
static int serial_imx_resume(struct platform_device *dev)
{
    ...
    /* when resume check bt on/off status then set pinctrl to uart mode */
	if (11 == gptHWCFG->m_val.bWifi || 12 == gptHWCFG->m_val.bWifi || 14 == gptHWCFG->m_val.bWifi) {
		if (gi_BT_power_status == false) {
			bt_pin_set(false);
		} else {
			bt_pin_set(true);
		} 
		...
		struct platform_device *pdev)
{
	...
	if (of_get_property(np, "fsl,bt-port-host", NULL)) {
		//bt_port_host = pdev;
        bt_port_host = sport;
		dev_info(&pdev->dev, "assigned as bt port host\n");
	}
...
static int serial_imx_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(sport->pinctrl)) {
		ret = PTR_ERR(sport->pinctrl);
		dev_err(&pdev->dev, "get pinctrl failed\n");
		return ret;
	}
	
	sport->pins_default = pinctrl_lookup_state(sport->pinctrl,
						PINCTRL_STATE_DEFAULT);
	if (IS_ERR(sport->pins_default))
		dev_warn(&pdev->dev, "could not get default state\n");

	sport->pins_sleep = pinctrl_lookup_state(sport->pinctrl,
						PINCTRL_STATE_SLEEP);
	if (IS_ERR(sport->pins_sleep))
		dev_warn(&pdev->dev, "could not get sleep state\n");
