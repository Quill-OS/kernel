...
struct ricoh61x_battery_info {
	...
	int temp_high_lv3_alarm;
        ...
        int iChargerDetectPostpone;
	int iChargeDetectDelayms;
...
static int _config_ricoh619_charger_params(struct device *bat_dev,int iChargerType)
{
	...
	val &= 0xe0;
	//if (status&0x30)
	if(iChargerType==CDP_CHARGER||iChargerType==DCP_CHARGER||iChargerType==SDP_OVRLIM_CHARGER)
	{	// DCP(10) or CDP(01) .
		...
		case 75: //E80K02
			...
			break;
		case 101: //E70K14
			ilim_adp = 0x13;	//2000mA
			ilim_usb = 0x13;	//2000mA
			ichg = 0x0E;		//1500mA
			break;
		default:
			ilim_adp = 0x0D;  	//1400mA
		...
		if(SDP_OVRLIM_CHARGER==iChargerType) {
			int i,iTryCnt=5;
			printk(KERN_INFO "PMU:%s set SDP over limit charging.\n", __func__);
			for(i=1;i<=iTryCnt;i++) {
				ricoh61x_write(bat_dev->parent, REGISET2_REG, ilim_usb|0xe0);// 0xb7
				ricoh61x_read(bat_dev->parent, REGISET2_REG, &val);
				if((val&0x1f)==ilim_usb) {
					break;
				}
				else {
					printk(KERN_DEBUG "PMU:%s wanna set ILIM_USB 0x%02x, but missmatch 0x%02x,retry %d/%d \n", __func__,
							ilim_usb,val,i,iTryCnt);
				}
				msleep(200);
			}
			...
			if(i>iTryCnt) {
				ricoh61x_read(bat_dev->parent, REGISET2_REG, &val);
				printk(KERN_ERR"PMU:%s times out to set sdp overlimit params ! reg0xb7=0x%x\n",__func__,val);
			}

			//ricoh61x_write(bat_dev->parent, REGISET2_REG, 0xe4); // 0xb7
		}
		else {
			printk(KERN_INFO "PMU:%s set DCP/CDP charging.\n", __func__);
		}
	...
	{	// SDP PC
		...
		if (gptHWCFG->m_val.bPCB >= 101) {
			if (iChargerType == NO_CHARGER_PLUGGED)
				ricoh61x_write(bat_dev->parent, REGISET2_REG, 0x20);
			else
...
#if 1
		printk("%s():SDP_OVRLIM enabled!\n",__func__);
#else 
		sprintf (buf,"SDP_OVRLIM\n");
		break;
#endif
	case SDP_PC_CHARGER:
		sprintf (buf,"SDP_PC\n");
		       ...
		       const char *buf, size_t count)
{
	...
	else if(0==strcmp(buf,"SDP_OVRLIM")) {
		giRICOH619_DCIN=SDP_OVRLIM_CHARGER;
	}
...
static void genOCVU10per_dynamic_tab(struct ricoh61x_battery_info *info,int voltage)
{
	...
	int u10_sqrt[11] = {0,400,600,715,800,835,850,880,930,990,1000};
...
static int ricoh61x_init_charger(struct ricoh61x_battery_info *info)
{
	...
	// 
	if(get_OCV_voltage(info, 10, ORIGINAL)>=4260000) {
		printk(" ****** Set battery full value 4.35V parameters ****** \n");
		info->ch_vfchg = 4;
		info->ch_vrchg = 4;
		info->jt_vfchg_h = 4;
		info->ch_vbatovset = 1;
	...
	if (gptHWCFG->m_val.bPCB >= 101) { 
		val &= 0x3F;		// Set  SDPOVRLIM to allow charge limit 500mA 
	} else {
	...
	if(info->iEnableTempDetect) {
		...
		val = info->temp_high_lv1_charge * 255 / 2500;
...
{
	return ((DCP_CHARGER == giRICOH619_DCIN))?0:1;
}
...
//	mutex_lock(&info->lock);
	...
	//printk(KERN_ERR"[ep0 detected] %s_%d , giRICOH619_DCIN:%d \n",__FUNCTION__,__LINE__,giRICOH619_DCIN);
	...
	info->iChargerDetectPostpone = 0;
	info->iChargeDetectDelayms = 500;
...
#define __charger_isr_func(info) \
{\
	if(delayed_work_pending(&info->irq_work)) {\
		printk("%s(%d):irq_work is pending !\n",__func__,__LINE__);\
	}\
	else {\
		printk("%s(%d)\n",__func__,__LINE__);\
		if(info->iChargerDetectPostpone) {\
			queue_delayed_work(info->workqueue, &info->irq_work,2*HZ);\
		}\
		else {\
			if(info->iChargeDetectDelayms == 1000) {\
				queue_delayed_work(info->workqueue, &info->irq_work,info->iChargeDetectDelayms);\
			}\
			else {\
				queue_delayed_work(info->workqueue, &info->irq_work,1);\
			}\
		}\
	}\
}
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
...
#if 1
		else if((SDP_PC_CHARGER==giRICOH619_DCIN)||(SDP_OVRLIM_CHARGER==giRICOH619_DCIN)) {
			val->intval = 2;
		}
#else 
		...
		else if(SDP_OVRLIM_CHARGER==giRICOH619_DCIN) {
			val->intval = 5;
		}
#endif
		else if(NO_CHARGER_PLUGGED==giRICOH619_DCIN) {
			val->intval = 0;
		}
		else if(-1==giRICOH619_DCIN) {
			val->intval = 0;
		}
		else {
			val->intval = 1;
...
static int ricoh61x_battery_probe(struct platform_device *pdev)
{
	...
	info->present = 1;
	info->iChargerDetectPostpone = 0;
	info->iChargeDetectDelayms  = 1000;	//delay for first boot
	...
	if( (74==gptHWCFG->m_val.bPCB) || (101==gptHWCFG->m_val.bPCB) ) {
		// E60K0X .
		giU10PtableType = OCV_UNDER10P_DYNAMIC_LINEAR;
	}
	/*
	else if((101==gptHWCFG->m_val.bPCB)) {
		// E70K1X .
		giU10PtableType = OCV_UNDER10P_DYNAMIC_EXP;
	}
	*/

	if(giU10PtableType>OCV_UNDER10P_DEFAULT) {
		printk("%s using ocv u10p dynamic table %d \n", __func__,giU10PtableType);
	}
...
//						"r5t61x_adc_vsysl", info);
	...
	if(info->iEnableTempDetect) {
		switch(gptHWCFG->m_val.bPCB) {
			...
			case 74: // E60K0X 
				//info->temp_high_lv1_charge = 852; // 38 degrees .
				info->temp_high_lv2_stop_charge = 700; // 44 degrees .
				//info->temp_high_lv3_alarm = 545; // 50 degrees .
				break;
			default:
				break;
...
static void ricoh61x_battery_shutdown(struct platform_device *pdev)
{
	struct ricoh61x_battery_info *info = platform_get_drvdata(pdev);

	printk("%s()\n",__func__);

	if (bat_alert_req_flg) {
		if(delayed_work_pending(&info->displayed_work)) {
			dev_info(info->dev,"%s : displayed_work pending ...\n",__func__);
			cancel_delayed_work_sync(&info->displayed_work);
		}
		dev_info(info->dev,"%s() finish displayed_work at first attach \n",__func__);
		ricoh61x_displayed_work(&info->displayed_work.work);
	}

	if(1==giCapacityMethod) {
		int soc_voltage;
		// voltaget method . 
		soc_voltage = calc_soc_by_voltageMethod(info);
		update_rsoc_on_voltageMethod(info, soc_voltage);
		dev_warn(info->dev,"update soc(%d) by voltage method\n",soc_voltage);
	}
...
static struct platform_driver ricoh61x_battery_driver = {
	...
	.shutdown	= ricoh61x_battery_shutdown,
};
