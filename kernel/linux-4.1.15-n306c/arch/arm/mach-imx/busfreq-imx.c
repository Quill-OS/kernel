...
static int giLowBufFreqDelayus=500000;
...
static void imx6ull_lower_cpu_rate(bool enter)
{
	...
	if (IS_ERR(arm_reg) || IS_ERR(soc_reg)) {
		printk ("[%s-%d] arm_reg %X , soc_reg %X not registered.\n", __func__, __LINE__, arm_reg, soc_reg);
		return;
	}
...
static int busfreq_probe(struct platform_device *pdev)
{
	...
	err = sysfs_create_file(&busfreq_dev->kobj, &dev_attr_mode.attr);
	if (err) {
		dev_err(busfreq_dev,
		       "Unable to register sysdev entry for freq mode");
		return err;
	}
