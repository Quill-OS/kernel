...
//						 __func__, i, int_sts[i], irq_mon_add[i]);
		...
		{
			u8 int_en_sys,pwron_times;
			ricoh61x_read(ricoh61x->dev,RICOH61x_INT_EN_SYS, &int_en_sys);
			ricoh61x_read(ricoh61x->dev,RICOH61x_PWR_ON_TIMSET, &pwron_times);
			printk(KERN_INFO"IRQ gpio=%d, IN_TEN_SYS=0x%x,PWR_ON_TIMES=0x%x,int_sts=0x%x,when interrupt is comming\n",
					gpio_get_value(ricoh61x->irq_gpio),int_en_sys,pwron_times,int_sts[i]);
				...
				struct ricoh619_platform_data *pdata)
{
	...
	if (pdata->irq_gpio && !ricoh61x->chip_irq) {
		...
		ricoh61x->irq_gpio = pdata->irq_gpio;
...
int ricoh61x_irq_exit(struct ricoh61x *ricoh61x)
{
	...
	if (ricoh61x->irq_gpio)
		gpio_free(ricoh61x->irq_gpio);
	return 0;
}
