...
static void mx50_print_lpddr2_mfgid(void)
{
	unsigned long mfgid =0x0;
	char *mfgname = "Reserved";

	if(!databahn_base) {
		printk(KERN_ERR "%s: databahn_base is NULL\n", __func__);
		return;
	}
	__raw_writel(0x00010005, (databahn_base + 0x64));
	mfgid = __raw_readl((databahn_base + 0x68));

	if(unlikely(mfgid >= ARRAY_SIZE(lpddr2_mfgids))) {
		switch(mfgid) {
		case 0xFE:
			mfgname = "Numonyx";
			break;	
		...
		mfgname = lpddr2_mfgids[mfgid];		
	}

	printk(KERN_ERR "MX50 LPDDR2 MfgID: 0x%lx [%s]\n", mfgid, mfgname);
	return;
}
