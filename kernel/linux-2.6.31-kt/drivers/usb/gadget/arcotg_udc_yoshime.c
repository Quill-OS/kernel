 ...
 * Copyright (C) 2009-2012 Amazon Technologies Inc. All Rights Reserved.
 */
...
#define	DRIVER_VERSION	"26 Mar 2012"
...
#define GADGET_LPM_TIMER msecs_to_jiffies(4000)
...
static void dr_phy_low_power_mode(struct fsl_udc *udc, bool enable)
{
	...
	/* Delay after coming out of low power mode to avoid spurious wakeups */
	if (!enable)
		udelay(700);
...
static void fsl_gadget_lpm_event(struct work_struct *work)
{
	...
	/* If vbus is lost along with D+/D- signals, usb_state won't enter 
	 * suspend state, which will prevent the system from suspending */
...
static bool fsl_vbus_irq(struct fsl_udc *udc)
{
	u32 irq_src;
	...
	if (irq_src & OTGSC_B_SESSION_VALID_IRQ_STS) {
		...
		/* Make sure the lpm task is not pending */
		cancel_delayed_work(&udc->gadget_lpm_work);
		
		fsl_writel(irq_src, &dr_regs->otgsc);
		/* only handle device interrupt event */
		if (!(fsl_readl(&dr_regs->otgsc) & OTGSC_STS_USB_ID)) {
		    pr_debug("%s: not device interrupt\n", __func__);
		    return true;
		}
		...
		if (irq_src & OTGSC_B_SESSION_VALID) {
			...
			/*let the system pm resume the udc if suspended */ 
			if (udc->suspended)
				return false;
			...
			schedule_delayed_work(&udc->gadget_lpm_work, GADGET_LPM_TIMER);
		}
...
static void fsl_gadget_detect_event(struct work_struct *work)
{
	struct fsl_udc *udc = udc_controller;
	struct fsl_usb2_platform_data *pdata;
	...
	/* Check to see if we have hit lpm mode yet */
	if (pdata->is_wakeup_event(pdata)) {
		
	    pr_debug("%s: wake up\n", __func__);
	
	    /* Need to wake up */
	    pdata->wakeup_handler(pdata);
	    pdata->lowpower = 0;
	    
	} else {
	    /* Spurious interrupt, turn off again */
	    dr_clk_gate(false);
	    pmic_vusb2_enable_vote(0, VUSB2_VOTE_SOURCE_OTG);
	    goto out;
	}
...
static irqreturn_t fsl_udc_irq(int irq, void *_udc)
{
	...
	/* Wake up the controller if it's asleep */
	if (pdata->lowpower) {
	    pr_debug("%s: detect\n", __func__);
	    disable_irq_nosync(udc->irq);
	    schedule_work(&udc->gadget_detect_work);
	    goto irq_end;
	} 

	/* Check for cable insert/remove events */
	if (fsl_vbus_irq(udc)) {
	    goto irq_end;
	}
	...
	if (irq_src & USB_STS_SUSPEND) {
		pr_debug("suspend int: %d", udc->usb_state);
		if (!(udc->usb_state == USB_STATE_SUSPENDED))
			suspend_irq(udc);
		status = IRQ_HANDLED;
	}
...
int usb_gadget_unregister_driver(struct usb_gadget_driver *driver)
{
	...
	pr_debug("%s: begin %d\n", __func__, udc_controller->stopped);

	cancel_delayed_work(&udc_controller->gadget_lpm_work);
...
static int __init fsl_udc_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&udc_controller->gadget_detect_work, fsl_gadget_detect_event);
	INIT_DELAYED_WORK(&udc_controller->gadget_lpm_work, fsl_gadget_lpm_event);
...
#if !(defined CONFIG_ARCH_MX35 || defined CONFIG_ARCH_MX25)
{
	...
	/*
     * TODO: This number needs to be verify again for celeste devices.  
     * */
...
static int fsl_udc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (((!(udc_controller->gadget.is_otg)) ||
		...
		pr_err("%s: is_otg=%d, otgsc=0x%x, usb_state=%d\n",
			__func__,udc_controller->gadget.is_otg, fsl_readl(&dr_regs->otgsc), udc_controller->usb_state);
