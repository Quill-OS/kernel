...
#include "ehci.h"
#include "ehci-dbg.c"
#define dbg2(dev, format, arg...)            \
        dev_printk(KERN_DEBUG ,ehci_to_hcd(dev)->self.controller , format , ## arg)
...
static void ehci_low_power_enter(struct ehci_hcd *ehci)
{
	...
	if (wakeup_value == 1) {
		dbg2(ehci,"USB entering idle mode\n");
		spin_lock_irq(&ehci->lock);
...
{	
	...
	dbg2(ehci_low_power,"USB leaving idle mode\n");
