...
static DEFINE_SPINLOCK(i2c_stop_lock);
	...
	for (i = 0; i < num && ret >= 0; i++) {
		...
		if (msgs[i].flags & I2C_M_RD) {
			...
			ret = mxc_i2c_readbytes(dev, &msgs[i], (i + 1 == num),
						addr_comp);
			if (ret < 0) {
				dev_err(&dev->adap.dev, " E mxc_i2c_xfer:readbytes:ret=%d:fail:addr0x%x\n", ret, msgs[i].addr);
				break;
			}
		} else {
			/* Write the data */
			...
			if (ret < 0) {
				dev_err(&dev->adap.dev, " E mxc_i2c_xfer:writebytes:ret=%d:fail:addr=0x%x\n", ret, msgs[i].addr);
				break;
			}
		}
	}
