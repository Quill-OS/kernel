...
PMIC_STATUS mc34708_bklit_set_hi_current(enum lit_channel channel, int mode)
{
	unsigned int mask;
	unsigned int value;
	int reg;
...
PMIC_STATUS mc34708_bklit_get_hi_current(enum lit_channel channel, int *mode)
{
	unsigned int mask;
	int reg;
				      ...
				      unsigned char level)
{
	...
	if (level > LIT_CURR_HI_42)
		...
		CHECK_ERROR(mc34708_bklit_set_hi_current(channel, 1));
		level -= LIT_CURR_HI_0;
	}
				      ...
				      unsigned char *level)
{
	...
	CHECK_ERROR(mc34708_bklit_get_hi_current(channel, &mode));
...
EXPORT_SYMBOL(mc34708_bklit_set_current);
EXPORT_SYMBOL(mc34708_bklit_get_current);
EXPORT_SYMBOL(mc34708_bklit_set_dutycycle);
EXPORT_SYMBOL(mc34708_bklit_get_dutycycle);
EXPORT_SYMBOL(mc34708_bklit_set_ramp);
EXPORT_SYMBOL(mc34708_bklit_get_ramp);
EXPORT_SYMBOL(mc34708_bklit_set_blink_p);
EXPORT_SYMBOL(mc34708_bklit_get_blink_p);
...
static int cmd(unsigned int index, int value)
{
	...
	case SET_CURR:
		...
		ret = mc34708_bklit_set_current(ch, value);
		break;
	case SET_DC:
		pr_debug("set %d dc %d\n", ch, value);
		ret = mc34708_bklit_set_dutycycle(ch, value);
		break;
	case SET_RAMP:
		pr_debug("set %d ramp %d\n", ch, value);
		ret = mc34708_bklit_set_ramp(ch, value);
		break;
	case SET_BP:
		pr_debug("set %d bp %d\n", ch, value);
		ret = mc34708_bklit_set_blink_p(ch, value);
		break;
	default:
