...
#define MUSBCHARG_LSH 11
#define MUSBCHARG_WID 1
...
#define USB_UART_CABLE 0xB20000
#define USB_UART_JIG2  0x720000

/* manual switching for usb/uart */
#define USB_UART_MANUAL_SW (0xB6C08D)

/* auto switching when vbus goes low */
#define USB_AUTO_SWITCH (0x80880F)
...
static ssize_t
show_charge_en(struct device *dev, struct device_attribute *attr, char *buf);

static ssize_t
store_charge_en(struct device *_dev, struct device_attribute *attr,
			const char *buf, size_t count);

static DEVICE_ATTR(charge_en, 0644, show_charge_en, store_charge_en);
...
{
	/* Cap the input charging current limit */
	CHECK_ERROR(pmic_write_reg(MC34708_REG_USB_CTL,
				   BITFVAL(MUSBCHARG,1) | BITFVAL(SWHOLD,0),
				   BITFMASK(MUSBCHARG) | BITFMASK(SWHOLD)));
	...
	/* set time delay to start powered accessory to 1600ms */
	//CHECK_ERROR(pmic_write_reg(MC34708_REG_USB_TIMING, BITFVAL(USBTD,0xf) ,BITFMASK(USBTD) ));
		...
		val =
		    BITFVAL(CHRCV,
			    CHRCV_UV_TO_BITS(di->
					     ...
					     [point].microVolt));
		pr_info("set charging point for usb type : %d\n",usb_type);
		switch (usb_type) {
		case USBHOST:
			/* set current limit to 100mA */

			CHECK_ERROR(pmic_write_reg(MC34708_REG_USB_CTL,
						   BITFVAL(MUSBCHRG, 1) | BITFVAL(MUSBCHARG,1),
						   BITFMASK(MUSBCHRG) | BITFMASK(MUSBCHARG)));
			val |= BITFVAL(CHRCC, CHRCC_UA_TO_BITS(250000));
/*
			CHECK_ERROR(pmic_write_reg(MC34708_REG_USB_CTL,
						   BITFVAL(MUSBCHRG, 2) | BITFVAL(MUSBCHARG,1),
						   BITFMASK(MUSBCHRG) | BITFMASK(MUSBCHARG)));
			val |= BITFVAL(CHRCC, CHRCC_UA_TO_BITS(450000));
*/
			break;
		case USBCHARGER:
...
#define ALL_CHARGER_TYPES	((USBHOST|USBCHARGER|DEDICATEDCHARGER|USB_UART_CABLE) & 0x3ffff)
#define USB_DETECT_RETRIES	(3)
static int ripley_charger_update_status(struct ripley_dev_info *di)
{
	...
	if (ret == 0) {
		...
		if (usbOnline != di->usb_charger_online) {
			usb_type = 0;
			/* check usb device type only if vbus is present */
			if( usbOnline ) {
				int i;
				for ( i=0 ; i<USB_DETECT_RETRIES ; i++ ) {
					/* need some delay to know the usb type */
					msleep(800);
					ret = pmic_read_reg(MC34708_REG_USB_DEVICE_TYPE,
						    &reg_usb_type, PMIC_ALL_BITS);
					if( usbOnline  && (reg_usb_type & ALL_CHARGER_TYPES) )
						break;
				}
				if( i == USB_DETECT_RETRIES )
					pr_warning("USB cable is connected, but could not detect device type\n");
				else
					pr_info("USB device type: %d detected within %d iterations\n",reg_usb_type,i+1);
				...
				} else if ( reg_usb_type == USB_UART_CABLE || reg_usb_type == USB_UART_JIG2 ) {
					usb_type = USB_UART_CABLE;
					pr_info("usb/uart cable attached!!!\n");
				}
			}
...
static void usb_charger_online_event_callback(void *para)
{
	...
	int ret;
	static int usbOnline = ~0U;
	unsigned int value;

	ret = ripley_charger_update_status(di);
	if (!ret) {
		if( usbOnline != di->usb_charger_online ) {
			usbOnline = di->usb_charger_online;
			if (usbOnline) {
				pr_debug("Charger Connected\n");
				kobject_uevent(&di->dev->kobj, KOBJ_ADD);
				if (device_create_file(di->dev, &dev_attr_connected) < 0)
					printk(KERN_ERR "Charger: Error creating device file\n");	
			}
			else {
				pr_debug("Charger Disconnected\n");
				pmic_write_reg(MC34708_REG_USB_CTL,
						   USB_AUTO_SWITCH,
						   0xffffff); 
				kobject_uevent(&di->dev->kobj, KOBJ_REMOVE);
				device_remove_file(di->dev, &dev_attr_connected);
			}
		}
	}
}
...
static void charger_timer_exp_event_callback(void *para)
{
	struct ripley_dev_info *di = (struct ripley_dev_info *)para;
	printk("Charger timer expires\n");
	ripley_charger_update_status(di);
}
...
static int charging_enabled(void)
{
	int reg_value = 0;
	pmic_read_reg(MC34708_REG_BATTERY_PROFILE,
		   &reg_value, BITFMASK(CHREN));
	return (!!reg_value);
}
...
static int charger_connected(void)
{
	...
	pmic_read_reg(MC34708_REG_INT_SENSE0, &sense_0, 0xffffff);
	if (BITFEXT(sense_0, USBDETS))
		ret = 1;
...
show_charging(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (charger_connected() && charging_enabled())
		return sprintf(buf, "1\n");
	else
		return sprintf(buf, "0\n");
}
...
static int ripley_battery_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_charge_en);
	kfree(di);
...
static int ripley_battery_probe(struct platform_device *pdev)
{
	...
	bat_event_callback.func = charger_timer_exp_event_callback;
	bat_event_callback.param = (void *)di;
	...
	if (device_create_file(&pdev->dev, &dev_attr_charge_en) < 0)
		printk(KERN_ERR "Charger: Error creating device file for charge_en\n");
