...
static int bq27541_battery_read_capacity(int *capacity)
{
	...
	err = bq27541_i2c_read(BQ27541_CSOC, &tmp);
	if (!err)
		*capacity = tmp;
...
static void bq27541_battery_read_lmd_cyc(int *lmd, int *cycl, int *cyct)
{
	...
	err1 = bq27541_i2c_read(BQ27541_LMD_L, &lo);
	err2 = bq27541_i2c_read(BQ27541_LMD_H, &hi);

	error = err1 | err2;

	if (!error)
		*lmd = ((((hi << 8) | lo) * 357) / (100 * BQ27541_BATTERY_RESISTANCE));

	/* Clear out hi, lo for next reading */
	hi = lo = 0;

	err1 = bq27541_i2c_read(BQ27541_CYCT_L, &lo);
	err2 = bq27541_i2c_read(BQ27541_CYCT_H, &hi);
