			 ...
			 unsigned long arg)
{
	...
	case RTC_AIE_OFF:
		pr_debug("alarm off\n");
		CHECK_ERROR(pmic_write_reg(REG_MC34708_RTC_ALARM, 0x100000, 0x100000));
		return 0;
	case RTC_AIE_ON:
		pr_debug("alarm on\n");
		CHECK_ERROR(pmic_write_reg(REG_MC34708_RTC_ALARM, 0, 0x100000));
		return 0;
	}
...
static int mxc_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	do {
		...
		CHECK_ERROR(pmic_read_reg(REG_MC34708_RTC_TIME, &value, mask));
...
static int mxc_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	CHECK_ERROR(pmic_read_reg(REG_MC34708_RTC_ALARM, &value, mask));
	tod_reg_val = BITFEXT(value, RTCALARM_TIME);

	mask = BITFMASK(RTCALARM_DAY);
	CHECK_ERROR(pmic_read_reg(REG_MC34708_RTC_DAY_ALARM, &value, mask));
...
static int mxc_rtc_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "mc34708 rtc probe start\n");
	...
	printk(KERN_INFO "mc34708 rtc probe succeed\n");
...
MODULE_DESCRIPTION("MC34708 Realtime Clock Driver (RTC)");
MODULE_LICENSE("GPL");
