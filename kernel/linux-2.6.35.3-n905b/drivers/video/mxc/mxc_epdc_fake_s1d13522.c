...
static int k_set_temperature(struct fb_info *info)
{
	...
	unsigned long dwCurrentTick=jiffies;

	//printk("%s(),timeout_ms=%d,current_ms=%d\n",__FUNCTION__,
	//		jiffies_to_msecs(gdwLastUpdateJiffies),jiffies_to_msecs(dwCurrentTick));
	
	if(0==gdwLastUpdateJiffies||time_after(dwCurrentTick,gdwLastUpdateJiffies)) {
		iChk = lm75_get_temperature(0,&giLastTemprature);
		if(iChk>=0) {
			iChk = mxc_epdc_fb_set_temperature(giLastTemprature,info);
			gdwLastUpdateJiffies = dwCurrentTick+msecs_to_jiffies(60000);
		}
	}
