...
static void mma8652_work_func(struct work_struct *work)
{
	...
	if (int_src & 0x08) {	// Pulse triggered
		...
		printk("[mma8652] SumX=%d SumY=%d SumZ=%d\n", SumX, SumY, SumZ); 
...
static int mma8652_probe(struct i2c_client *client,
				   const struct i2c_device_id *id)
{
	...
	// get data from dts
	int_gpio = of_get_named_gpio(np, "gpio_intr", 0);
	if (!gpio_is_valid(int_gpio))
	{
		printk("[mma8652] gpio is failed :%d \n",int_gpio);
		return -ENODEV;
	}
	printk("[mma8652] gpio(%d) to irq(%d) \n",int_gpio,client->irq);
...
static int mma8652_remove(struct i2c_client *client)
{
#if 0
	struct mma8652_data *pdata = i2c_get_clientdata(client);
