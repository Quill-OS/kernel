...
extern ktime_t ktime_get_last_resume_time(void) ;
			  ...
			  size_t count, loff_t *ppos)
{
	...
	int error;
	ktime_t ev_time[EV_CLK_MAX];
	ktime_t last_resume_time = ktime_get_last_resume_time();
	ktime_t evt_time_cur;
	ktime_t last_resume_time_for_comp;
	...
	for (;;) {
		...
		while (read + input_event_size() <= count &&
			...
			ev_time[EV_CLK_MONO] = last_resume_time;
			ev_time[EV_CLK_REAL] = ktime_mono_to_real(ev_time[EV_CLK_MONO]);
			ev_time[EV_CLK_BOOT] = ktime_mono_to_any(ev_time[EV_CLK_MONO],TK_OFFS_BOOT);
			last_resume_time_for_comp = ev_time[client->clk_type];

			evt_time_cur = timeval_to_ktime(event.time);
#if 0
			printk("%s():last resume ktime=%llu,last resume ktime comp=%llu,evt ktime=%llu\n",__func__,\
					last_resume_time.tv64,last_resume_time_for_comp.tv64,evt_time_cur.tv64);
#endif
			if( ktime_compare(evt_time_cur,last_resume_time_for_comp) < 0 ) {
#if 0
				printk("dirty evt found,patch it! evttype=0x%02x,evcode=0x%02x,evtval=%d\n",
						event.type,event.code,event.value);
#endif
				event.time = ktime_to_timeval(last_resume_time_for_comp);
			}
