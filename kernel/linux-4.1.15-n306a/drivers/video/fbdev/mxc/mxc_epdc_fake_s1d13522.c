...
#ifdef JD9930_ENABLED//[
	#include <linux/mfd/jd9930.h>
#endif //]JD9930_ENABLED
...
static DEVICE_ATTR(waveform_mode_du4, S_IWUSR | S_IRUSR|S_IROTH|S_IRGRP, waveform_mode_read, waveform_mode_write);
static DEVICE_ATTR(waveform_mode_gck16, S_IWUSR | S_IRUSR|S_IROTH|S_IRGRP, waveform_mode_read, waveform_mode_write);
static DEVICE_ATTR(waveform_mode_glkw16, S_IWUSR | S_IRUSR|S_IROTH|S_IRGRP, waveform_mode_read, waveform_mode_write);
...
static DEVICE_ATTR(power_state, S_IWUSR|S_IRUSR|S_IROTH|S_IRGRP, power_state_read, power_state_write);
...
#ifdef JD9930_ENABLED//[
//const char *mx6sll_jd9930_node_name="/soc/aips-bus@02100000/i2c@021a4000/jd9930@18";
const char *mx6sll_jd9930_node_name="/soc/aips-bus@02100000/i2c@021a4000/jd9930@18";
const char *mx6ull_jd9930_node_name="/soc/aips-bus@02100000/i2c@021a0000/jd9930@18";
static struct jd9930 *_get_jd9930(void)
{
	struct i2c_client *jd9930_i2c_client;			
	struct jd9930 *jd9930 = 0;
	struct device_node *np;
	const char *node_name = mx6sll_jd9930_node_name;
	...
	if(9==gptHWCFG->m_val.bCPU) {
		// MX6ULL .
		node_name = mx6ull_jd9930_node_name;
	}
	else 
	if(10==gptHWCFG->m_val.bCPU) {
		// MX6SLL
		node_name = mx6sll_jd9930_node_name;
	}
	else {
		node_name = mx6sll_jd9930_node_name;
		printk(KERN_ERR"%s():CPU type (%d) cannot be recognized !",
	...
	if(np) 
	{
		jd9930_i2c_client = of_find_i2c_device_by_node(np);
		jd9930 = i2c_get_clientdata(jd9930_i2c_client);
...
static ssize_t vcom_read(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	{
		// 
		iChk = jd9930_get_vcom(g_jd9930,&iVCOM_mV);
		if(iChk>=0) {
			sprintf (buf,"%dmV\n",iVCOM_mV);
		}
	}
		       ...
		       const char *buf, size_t count)
{
	...
	if(iVCOM_mV<0) {
		...
		{
			// 
			iChk = jd9930_set_vcom(g_jd9930,iVCOM_mV,0);
			if(iChk<0) {
				printk(KERN_ERR"JD9930 VCOM %dmV write failed !\n",iVCOM_mV);
			}
		}
		else 
#endif //]JD9930_ENABLED
...
static const char gszNightModeTest[]="night-mode-test";
static ssize_t dbg_read(struct device *dev, struct device_attribute *attr,char *buf)
{
	int tot_len=0;
	int len;

	len = sprintf(buf+tot_len,"%d\n",g_fb_data->verbose_lvl);tot_len+=len;

#ifdef NIGHT_MODE_SUPPORT //[
	len = sprintf(buf+tot_len,"%s=%d (1:nightmode timing; 0:xon force off;-1:xon force on;-2:don't controll)\n",gszNightModeTest,g_fb_data->night_mode_test);tot_len+=len;
		       ...
		       const char *buf, size_t count)
{
	...
	iLenBuf=iLen=strlen(buf);
	
	if(iLenBuf>=256) {
		dev_err(dev,"dbg command string length cannot >= 256\n");
	}	

	//printk(KERN_ERR"%s() : strlen=%d, 0x%02x\n",__func__,iLenBuf,(unsigned char)buf[iLenBuf]);

	if( '\x0a'==buf[iLenBuf] || '\x0d'==buf[iLenBuf] || '\0'==buf[iLenBuf] ) {
		iLenBuf -= 1;
	}
	strncpy(cBufA,buf,iLenBuf);
	cBufA[iLenBuf]='\0';
	pcBuf = &cBufA[0];
	...
	if (*pcBuf < '0' || *pcBuf > '9') {
		...
		if(0==strcmp(pcBuf,"epdc-reinit")) {
			epdc_recovery(g_fb_data);
		}
#ifdef NIGHT_MODE_SUPPORT //[
		else if(strstr(pcBuf,gszNightModeTest)) {
			pcTmp=pcBuf+strlen(gszNightModeTest)+1;
			if( '\0'==*pcTmp ) {
				if(g_fb_data->night_mode_test) {
					g_fb_data->night_mode_test = 0;
				}
				else {
					g_fb_data->night_mode_test = 1;
				}
			}
			else {
				if(0==strcmp(pcTmp,"invert")) {
					g_fb_data->force_invert = g_fb_data->force_invert?0:1;
					return iLen;
				}
				else
				if('0'==pcTmp[0] && 'x'==pcTmp[1]) {
					g_fb_data->night_mode_test = simple_strtol(pcTmp+2,0,16);
				}
				else {
					g_fb_data->night_mode_test = simple_strtol(pcTmp,0,10);
				}
			}

			if(g_fb_data->night_mode_test>=1) {
				g_fb_data->vcom_off_with_data = 1;
			}
			else {
				g_fb_data->vcom_off_with_data = 0;
			}
			if(g_fb_data->night_mode_test>=2) {
				g_fb_data->force_invert = 1;
			}
			else {
				g_fb_data->force_invert = 0;
			}
			#ifdef WFM_ENABLE_NIGHTMODE //[
			if(g_fb_data->night_mode_test>=3) {
				// gck pretend gc16 and aad . 
				// glkw pretend gl16 and aa .
				giNTX_waveform_modeA[NTX_WFM_MODE_GC16] = g_fb_data->wv_modes.mode_gck16;
				giNTX_waveform_modeA[NTX_WFM_MODE_GL16] = g_fb_data->wv_modes.mode_glkw16;
				giNTX_waveform_modeA[NTX_WFM_MODE_GLR16] = g_fb_data->wv_modes.mode_glkw16;
				giNTX_waveform_modeA[NTX_WFM_MODE_GLD16] = g_fb_data->wv_modes.mode_gck16;
			}
			else 
			#endif //] WFM_ENABLE_NIGHTMODE
			{
				giNTX_waveform_modeA[NTX_WFM_MODE_GC16] = g_fb_data->wv_modes.mode_gc16;
				giNTX_waveform_modeA[NTX_WFM_MODE_GL16] = g_fb_data->wv_modes.mode_gl16;
				giNTX_waveform_modeA[NTX_WFM_MODE_GLR16] = g_fb_data->wv_modes.mode_aa;
				giNTX_waveform_modeA[NTX_WFM_MODE_GLD16] = g_fb_data->wv_modes.mode_aad;
			}
			dev_info(dev,"%s=%d\n",gszNightModeTest,g_fb_data->night_mode_test);
		}
#endif //] NIGHT_MODE_SUPPORT
	}
	else {
...
static ssize_t waveform_mode_read(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	if(attr==&dev_attr_waveform_mode_du4.attr) {
		iWaveform_mode_eink = giNTX_waveform_modeA[NTX_WFM_MODE_DU4];
		iWaveform_mode = NTX_WFM_MODE_DU4;
	}
	else
	if(attr==&dev_attr_waveform_mode_gck16.attr) {
		iWaveform_mode_eink = giNTX_waveform_modeA[NTX_WFM_MODE_GCK16];
		iWaveform_mode = NTX_WFM_MODE_GCK16;
	}
	else
	if(attr==&dev_attr_waveform_mode_glkw16.attr) {
		iWaveform_mode_eink = giNTX_waveform_modeA[NTX_WFM_MODE_GLKW16];
		iWaveform_mode = NTX_WFM_MODE_GLKW16;
	}
		       ...
		       const char *buf, size_t count)
{
	...
	case NTX_WFM_MODE_DU4:
	case NTX_WFM_MODE_GCK16:
	case NTX_WFM_MODE_GLKW16:
...
static const struct attribute *sysfs_epdc_attrs[] = {
	...
	&dev_attr_waveform_mode_du4.attr,
	&dev_attr_waveform_mode_gck16.attr,
	&dev_attr_waveform_mode_glkw16.attr,
#ifdef OUTPUT_SNAPSHOT_IMGFILE//[
...
static int k_set_temperature(struct fb_info *info)
{
	...
	if(0==gdwLastUpdateJiffies||time_after(jiffies,gdwLastUpdateJiffies)) {
		...
		{
			// 
			iChk = jd9930_get_temperature(g_jd9930,&giLastTemprature);
		}
		else
#endif //]JD9930_ENABLED
		{
...
static int k_set_vcom(int iVCOM_set_mV)
{
	...
	if(g_jd9930)
	{
		iRet = jd9930_set_vcom(g_jd9930,iVCOM_set_mV,0);
		if(iRet>=0) {
			int iVCOM_get_mV;
			if(jd9930_get_vcom(g_jd9930,&iVCOM_get_mV)>=0) {
				vcom_nominal=iVCOM_get_mV*1000;/* save the vcom_nominal value in uV */
			}
		}
	}
...
static int k_set_vcom_to_flash(int iVCOM_set_mV)
{
	...
	if(g_jd9930)
	{
		iRet = jd9930_set_vcom(g_jd9930,iVCOM_set_mV,1);
		if(iRet>=0) {
			int iVCOM_get_mV;
			if(jd9930_get_vcom(g_jd9930,&iVCOM_get_mV)>=0) {
				vcom_nominal=iVCOM_get_mV*1000;/* save the vcom_nominal value in uV */
...
static int k_get_vcom(int *O_piVCOM_get_mV)
{
	...
	if(g_jd9930)
	{
		...
		iRet = jd9930_get_vcom(g_jd9930,O_piVCOM_get_mV);
	}
	else
#endif //]JD9930_ENABLED
	{
...
static void ntx_epdc_pmic_exception(int iEvt)
{
	if(7==gptHWCFG->m_val.bDisplayCtrl || 15==gptHWCFG->m_val.bDisplayCtrl||
			...
			26==gptHWCFG->m_val.bDisplayCtrl || 27==gptHWCFG->m_val.bDisplayCtrl) 
	{
		// MX6SL+TPS65185 | MX6SLL+TPS65185 | MX6SL+SY7636|MX6ULL+SY7636|MX6SLL+SY7636|MX6DL+SY7636 .
		// MX6SLL+JD9930 | MX6ULL+JD9930 
#ifdef TPS65185_ENABLED//[
		if(g_tps6518x)
		{
			// MX6SL + TPS65185 .
		...
		if(g_jd9930)
		{
			...
			//if(tps65185_int_state_get()>=0) 
			{
				jd9930_int_state_clear(g_jd9930);
			}
		}
#endif //] JD9930_ENABLED
	...
	if(gptDC) {
		...
		switch (gptHWCFG->m_val.bDisplayCtrl) {
			...
			case 27 : // MX6ULL+JD9930
				g_jd9930 = _get_jd9930();
				jd9930_int_callback_setup(g_jd9930,ntx_epdc_pmic_exception);
				break;
#endif //]JD9930_ENABLED
		}
			...
			
			{
					...
					{
						if(0!=glVCOM_uV) {
							if(jd9930_set_vcom(g_jd9930,(int)(glVCOM_uV/1000),0)<0) {
								WARNING_MSG("%s(),jd9930 vcom set to %d uV fail !\n",
									__FUNCTION__,glVCOM_uV);
							}
							else {
								DBG_MSG("%s():jd9930 set vcom to %d uV ok !\n",
									__FUNCTION__,glVCOM_uV);
							}
						}
						iChk = jd9930_get_vcom(g_jd9930,&vcom_mV);
					}
					else 
#endif //]JD9930_ENABLED
#ifdef TPS65185_ENABLED//[
