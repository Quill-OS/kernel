			...
			if(15==gptHWCFG->m_val.bDisplayResolution) {
				...
				if(0x0000==wRotate) {
					...
					if(0==gptHWCFG->m_val.bDisplayPanel||3==gptHWCFG->m_val.bDisplayPanel||\
							...
							9==gptHWCFG->m_val.bDisplayPanel||16==gptHWCFG->m_val.bDisplayPanel||35==gptHWCFG->m_val.bDisplayPanel)
					{
						// Left Out EPD Panel ...
	  ...
	  case S1D13521_DISPLAY:GALLEN_DBGLOCAL_RUNLOG(0);
		{
			...
			int iBpp = 4;
			ST_IMAGE_PGM *pt ;
			unsigned int dwImgValidSize;
	
			DBG_MSG("%s():S1D13521_DISPLAY\n",__FUNCTION__);
			dwImgValidSize=((sizeof(unsigned int)*6)+(pDC->dwWidth*pDC->dwHeight));
			if ( 0==gptHWCFG->m_val.bUIStyle ) {
				...
				pt = kmalloc(dwImgValidSize,GFP_KERNEL);
			}
			if(pt)
			{
				...
				copy_from_user(pt,(ST_IMAGE_PGM *)arg,dwImgValidSize);
				GALLEN_DBGLOCAL_PRINTMSG("DISPLAY INFO : X=%ld,Y=%ld,W=%ld,H=%ld\n\twaveform=%ld,LUT no.=%ld,datap=%p,size=%ld\n",
					...
					pt->WaveForm,pt->LUT_NO,pt->Data,dwImgValidSize);
