...
static int _any_chn_brightness_on(void)
{
	int iChipIdx;

	for(iChipIdx=0;iChipIdx<gilm3630a_chips;iChipIdx++) {
		//printk("chip%d,chA,brightness=%d\n",iChipIdx,gpchip[iChipIdx]->bBrightnessA);
		//printk("chip%d,chB,brightness=%d\n",iChipIdx,gpchip[iChipIdx]->bBrightnessB);
		if(gpchip[iChipIdx]->bBrightnessA) {
			//printk("chip%d,chA,brightness=%d\n",iChipIdx,gpchip[iChipIdx]->bBrightnessA);
			return 1;
		}
		if(gpchip[iChipIdx]->bBrightnessB) {
			//printk("chip%d,chB,brightness=%d\n",iChipIdx,gpchip[iChipIdx]->bBrightnessB);
			return 1;
		}
	}
...
static int lm3630a_bank_a_update_status(struct backlight_device *bl)
{
	...
	if (bl->props.state & BL_CORE_SUSPENDED) {
		...
		if (gSleep_Mode_Suspend) {
			...
			//printk("%s():suspend\n",__FUNCTION__);
			if (cpu_is_imx6ull() || 87==gptHWCFG->m_val.bPCB || 101==gptHWCFG->m_val.bPCB) {
				//fl_current_now = lm3630a_get_FL_current();
				//if (fl_current_now == -1 && fl_en_status) 
				if (fl_en_status && !_any_chn_brightness_on() )
				{
					printk("%s():FL PWR[OFF] \n",__FUNCTION__);
					gpio_direction_output(pchip->hwen_gpio, 0);
					if (gpio_is_valid(pchip->pwr_on_gpio)) {
	...
	else if ((cpu_is_imx6ull() || 87==gptHWCFG->m_val.bPCB || 101==gptHWCFG->m_val.bPCB) && giSuspendingState && !fl_en_status) {
		if (gpio_is_valid(pchip->pwr_on_gpio)) {
			gpio_direction_output(pchip->pwr_on_gpio,1);
		}
		gpio_direction_output(pchip->hwen_gpio, 1);
...
//#endif //]RGBW_BSEARCH_DBG
	...
	case 6:// 1 color FL @ channel 0(A).
		if (gptLm3630fl_dualcolor_percent_tab){
			// 1LM3630 * 1 using dualcolor firmware
			int iPercentIdx=gpchip[0]->bled->props.brightness-1;
			if(gpchip[0]->bled->props.brightness>0) {
				iRet = (int)gptLm3630fl_dualcolor_percent_tab[0].dwCurrentA[iPercentIdx];
				printk(KERN_ERR"dualcolor[%d][%d] curr=%d\n",0,iPercentIdx+1,iRet);
			}
			else {
				iRet = 0;
			}
		}	
		else if(gpchip[0]->bleda->props.brightness) {
			iTabIdx = gpchip[0]->bleda->props.brightness-1;
			if(lm3630a_get_FL_RicohCurrTab(0,0,&pdwCurrTab,&dwCurrTabSize)<0) {
		...
		unsigned char led_B_current, unsigned char led_B_brightness)
{
	...
	pchip->bleda->props.brightness = pchip->bBrightnessA = led_A_brightness;
	...
	pchip->bledb->props.brightness = pchip->bBrightnessB = led_B_brightness;
	...
	if(gilm3630a_chips>iChipIdx) {
		...
		if(5==gptHWCFG->m_val.bFL_PWM||6==gptHWCFG->m_val.bFL_PWM||7==gptHWCFG->m_val.bFL_PWM) {
			...
			else if(gptLm3630fl_dualcolor_percent_tab)
			{
				...
				if(6==gptHWCFG->m_val.bFL_PWM) {
					// 1 color ,white FL @ channel A .
					lm3630a_set_FL_EX (0,gptLm3630fl_dualcolor_percent_tab[0].bC1_CurrentA[iFL_Percentage-1],
									gptLm3630fl_dualcolor_percent_tab[0].bC1_BrightnessA[iFL_Percentage-1],0,0);
				}
			}
			else {
...
static void lm3630a_bl_parse_dt(struct lm3630a_chip *pchip)
{
	...
	pchip->pwr_on_gpio = of_get_named_gpio(lm3630a_bl_pmic_np,"pwron",0);
		if (!gpio_is_valid(pchip->pwr_on_gpio)) {
			printk("invalid gpio: %d\n",  pchip->pwr_on_gpio);
		}
			 ...
			 const struct i2c_device_id *id)
{
	...
	if(5==gptHWCFG->m_val.bFL_PWM||6==gptHWCFG->m_val.bFL_PWM||7==gptHWCFG->m_val.bFL_PWM) {
		...
		pchip->pbPercentDutyTableA = lm3630a_get_FL_W_duty_table(iTabIdx,&pchip->iPercentDutyTableASize);
		pchip->pbPercentDutyTableB = lm3630a_get_FL_W_duty_table(iTabIdx,&pchip->iPercentDutyTableBSize);
		lm3630a_get_default_power_by_table(iTabIdx,&pchip->bDefaultCurrentA);
		lm3630a_get_default_power_by_table(iTabIdx,&pchip->bDefaultCurrentB);
	}
