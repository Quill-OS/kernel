...
static int ntx_wifi_bt_power_init(void)
{
	...
	else if( 0x10 == gptHWCFG->m_val.bWifi && 0x06 == gptHWCFG->m_val.bPMIC){ // Wifi 88w8987 , PMIC:BD71828
		printk(KERN_ERR"[WIFI]  88W8987  !!\n");
		return 1 ;
	}
	else {
...
extern void bt_pin_set(bool);
extern int set_bd71828_Wifi_1V1(int status);
static void _ntx_wifi_bt_power_ctrl (int power_status)
{
	...
	if (power_status & 0x3)	{
		...
		if( 0x10 == gptHWCFG->m_val.bWifi && 0x06 == gptHWCFG->m_val.bPMIC){ // Wifi 88w8987 , PMIC:BD71828
			set_bd71828_Wifi_1V1(1);
		}
	} 
...
void ntx_wifi_power_ctrl(int iIsWifiEnable)
{
	...
	if (16 == gptHWCFG->m_val.bWifi || 11 == gptHWCFG->m_val.bWifi || 12 == gptHWCFG->m_val.bWifi || 14 == gptHWCFG->m_val.bWifi) { //sync the BT power status
		mutex_lock(&ntx_wifi_power_mutex);
		if (iIsWifiEnable)
...
#ifdef CONFIG_MFD_ROHM_BD71828 //[
extern int bd71828_restart(void);
extern int bd71828_power_off(void);
#endif //]CONFIG_MFD_ROHM_BD71828
...
//#endif //]CONFIG_MFD_RICOH619
//#ifdef CONFIG_MFD_ROHM_BD71828 //[
		if(6==gptHWCFG->m_val.bPMIC) {
			// BD71828
			printk("%s() --- BD71828 restarting system ---\n",__FUNCTION__);
			bd71828_restart();
		}
		else
//#endif //]CONFIG_MFD_ROHM_BD71828
...
const char bd7181x_bat_psy_name[] = "usb";
...
static int _Is_USB_plugged(void)
{
	...
	if(psy) {
		...
		if(6==gptHWCFG->m_val.bPMIC) {
			iChk = power_supply_get_property(psy,POWER_SUPPLY_PROP_ONLINE,&psy_propval);
			if(iChk>=0) {
				iRet = psy_propval.intval;
			}
			else {
				iRet = 0;
  ...
  //struct ebook_device_info info; 
	...
	switch(command)
	{
		...
		case POWER_KEEP_COMMAND:
			...
			gKeepPowerAlive=p;
			if(1==gptHWCFG->m_val.bPMIC||6==gptHWCFG->m_val.bPMIC)
				break;
			if (gKeepPowerAlive) {
#ifdef CONFIG_NTX_LED
