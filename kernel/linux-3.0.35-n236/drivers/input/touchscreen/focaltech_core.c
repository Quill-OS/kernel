...
extern int gIsCustomerUi;	// netronix customer UI flag.
extern int gSleep_Mode_Suspend;	// netronix hibernation flag.
...
static void fts_report_value(struct fts_ts_data *data)
{
	...
	u8 buf[POINT_READ_BUF] = { 0 };
	static uint16_t last_x[5], last_y[5];
	...
	for (i = 0; i < FTS_MAX_POINTS; i++) {
		...
		if (gIsCustomerUi) 
			event->au16_y[i] = (fts_wq_data->pdata->y_max-1) - event->au16_y[i];
	...
	for (i = 0; i < event->touch_point; i++)
	{
		if (event->au8_touch_event[i] == FTS_TOUCH_DOWN || event->au8_touch_event[i] == FTS_TOUCH_CONTACT)
		{
			...
			input_report_abs(data->input_dev, ABS_PRESSURE, event->pressure[i]);
		...
		else
		{
			...
			input_report_abs(data->input_dev, ABS_MT_POSITION_X, last_x[event->au8_finger_id[i]]);
			input_report_abs(data->input_dev, ABS_MT_POSITION_Y, last_y[event->au8_finger_id[i]]);
			input_mt_sync (data->input_dev);
#endif
			data->touchs &= ~BIT(event->au8_finger_id[i]);
			input_report_abs(data->input_dev, ABS_X, last_x[event->au8_finger_id[i]]);
			input_report_abs(data->input_dev, ABS_Y, last_y[event->au8_finger_id[i]]);
	...
	if(unlikely(data->touchs ^ touchs))
	{
		for(i = 0; i < FTS_MAX_POINTS; i++)
		{
			if(BIT(i) & (data->touchs ^ touchs))
			{
				...
				input_report_abs(data->input_dev, ABS_MT_TOUCH_MAJOR, 0);
				input_report_abs(data->input_dev, ABS_MT_PRESSURE, 0);
				input_report_abs(data->input_dev, ABS_MT_POSITION_X, last_x[i]);
				input_report_abs(data->input_dev, ABS_MT_POSITION_Y, last_y[i]);
				input_mt_sync (data->input_dev);
#endif
				input_report_abs(data->input_dev, ABS_X, last_x[i]);
				input_report_abs(data->input_dev, ABS_Y, last_y[i]);
				input_report_abs(data->input_dev, ABS_PRESSURE, 0);
			}
		}
	}
...
#endif
	if (gSleep_Mode_Suspend) { // hibernation
		free_irq(data->client->irq, data);	//release irq.
		if (data->pdata->power_on)
...
int fts_ts_resume(struct device *dev)
{
	...
	if (err < 0)
		return err;
#endif
	if (gSleep_Mode_Suspend) { // wakeup from hibernation
		if (fts_wq_data->pdata->power_on)
			err = data->pdata->power_on(true);

		request_threaded_irq(data->client->irq, NULL, fts_ts_interrupt,
				data->pdata->irqflags | IRQF_ONESHOT | IRQF_TRIGGER_FALLING,
				data->client->dev.driver->name, data);
		enable_irq(data->client->irq);
		data->suspended = false;
	}
	else if (!gpio_get_value(data->pdata->irq_gpio)) {
		queue_work(data->ts_workqueue, &data->touch_event_work);
	}
	return 0;
}
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	input_set_abs_params(input_dev, ABS_MT_WIDTH_MAJOR, 0, 2, 0, 0);
#endif
